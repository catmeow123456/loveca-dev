...
| `src/scripts/sync-cards-loveca-excel.ts`  | `docs/card-data-sync/sources/loveca_*.xlsx` 或 CloudBase 集合 | 在已有卡牌基础上同步来源权威的卡牌类型，并补强中日名称、中日效果、真实团体、真实小队、成员持有 Heart、BLADE Heart、LIVE 必要 Heart、商品编号、图片来源 URI 和外部来源标识 | 不插入 source-only 新卡；不删除 DB-only 卡；不覆盖费用、BLADE、LIVE 分数、`work_names`                                          |
