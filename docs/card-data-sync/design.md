...
- `llocg_db` 主同步：从 JP/CN JSON 读取结构化规则字段，并为新卡提供基础展示字段，转换为项目内部卡牌模型后写入 `cards` 表。
- Loveca 文本/来源同步：默认从 Loveca Excel 读取来源权威卡牌类型、中日名称、中日效果文本、真实团体、真实小队、成员持有 Heart、BLADE Heart、LIVE 必要 Heart 和商品来源字段，也可通过 `--source=cloudbase` 从腾讯云 CloudBase `loveca` 集合读取同等字段，写入对应字段；不读取 Excel 官方 `作品名` / `参加ユニット`，也不接管费用、BLADE、分数等其他对局规则字段。
...
| 脚本                                      | 上游数据源                                                    | 核心职责                                                                                                                                                                  | 不负责                                                                                                                          |
| ----------------------------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `src/scripts/sync-cards-llocg.ts`         | `llocg_db/json/cards.json`、`llocg_db/json/cards_cn.json`     | 建立或刷新卡牌主记录；负责卡牌类型、费用、Heart、BLADE、LIVE 分数、必要 Heart、图片文件名、稀有度和作品数组；新卡插入时写入初始中日文本与收录商品                         | 不读取 Loveca Excel；已有卡不覆盖中日名称、中日效果文本、收录商品、Excel 修正后的真实团体、商品编号、云端图链来源和外部数据标识 |
| `src/scripts/sync-cards-loveca-excel.ts`  | `docs/card-data-sync/sources/loveca_*.xlsx` 或 CloudBase 集合 | 在已有卡牌基础上同步来源权威的卡牌类型，并补强中日名称、中日效果、真实团体、真实小队、成员持有 Heart、BLADE Heart、LIVE 必要 Heart、商品编号、图片来源 URI 和外部来源标识 | 常规同步不插入 source-only 新卡；仅本地测试库显式传入 `--seed-missing-for-test --yes` 时可初始化缺失卡；不删除 DB-only 卡 |
...
推荐运行顺序是先运行 `sync-cards-llocg.ts` 建立规则字段和基础卡池，再运行 `sync-cards-loveca-excel.ts` 补齐更可靠的双语文本、真实团体、小队原文、商品和来源信息。后续再次运行 `sync-cards-llocg.ts` 时，已有卡会保留数据库中的 `name_jp` / `name_cn` / `card_text_jp` / `card_text_cn` / `product`，避免覆盖 Excel、CloudBase 或管理端已经修正过的展示和来源字段。
...
Loveca 文本/来源同步同样先标准化卡牌编号，但它只匹配数据库已有记录。来源类型必须映射为 `MEMBER`、`LIVE` 或 `ENERGY`；缺失或非法类型的整行会报告并跳过。source-only 卡号会报告并跳过，DB-only 卡不受影响；输入内部出现标准化重复卡号时，整组跳过并输出 warning，避免用遍历顺序决定覆盖结果。
...
- 基础身份：卡牌编号、卡牌类型；`llocg_db` 只在新卡插入时提供初始中日名称，Loveca Excel / CloudBase `loveca` 可校正已有卡的来源权威卡牌类型。
- 展示资料：`llocg_db` 只在新卡插入时提供初始中日效果文本和收录商品，已有卡保留数据库现值；图片文件名和稀有度仍可由 `llocg_db` 同步刷新。
...
未被同步覆盖的字段应由管理端或其他明确入口维护。已有卡的中日名称、中日效果文本和收录商品由 Loveca Excel / CloudBase / 管理端维护，`llocg_db` 只给新卡提供初始值。新增卡牌字段时，需要同时评估同步脚本、卡牌管理 API 和前端转换层。
...
CloudBase 新卡导入通过 `sync-cards-cloudbase-new.ts --cloudbase-collection=<collection>` 启用，同样读取 `CLOUDBASE_ENV_ID`、`CLOUDBASE_SECRET_ID`、`CLOUDBASE_SECRET_KEY`。当前确认可读取的卡牌集合为 `loveca`，也是脚本默认值；`real_card` 不存在。正式运行必须显式选择 `--upload-images` 或 `--skip-images`；`--upload-images` 还需要 `MINIO_ENDPOINT`、`MINIO_ACCESS_KEY`、`MINIO_SECRET_KEY` 等对象存储环境变量。每次导入会为卡图派生任务独立的版本化 basename，三个尺寸共用该版本；系统任务只在有效租约事务中把它写入 `image_filename`，并在 `source_flags` 中显式记录版本化标记与原始 basename，因此失租旧 worker 的延迟上传只能形成无引用的旧任务对象，不能覆盖新任务图片。这组标记是内部派生状态：CloudBase 和卡牌管理 API 中的同名外部键会被剔除，跳过图片、允许缺图、管理中心换图或旧同步脚本改变 `image_filename` 时会清除。候选与数据库比较图片 basename 时只读取与当前文件名严格对应的元数据，空图片名、部分标记或对应不一致会阻断整个预览，未标记文件名不剥离任何疑似后缀；数据库 `COMMIT` 返回异常时会尝试结束原事务并通过独立连接对账，只有回滚已确认且数据库未引用本任务文件名后才清理图片。图片失败默认阻止该卡插入，除非显式传入 `--allow-missing-images` 并在 source flags 中记录失败原因。`--skip-images` 不写入 `image_filename`，只保留 `image_source_uri`，避免前端把尚未上传的对象误判为可用卡图。
