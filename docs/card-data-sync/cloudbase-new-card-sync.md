...
每次正式执行都从任务身份派生独立的版本后缀，三个 WebP 尺寸写入 `${size}/${sourceBase}-${version}.webp`，并在元数据中保留各自的 SHA-256。同一执行只会复用同键且哈希与当前内容完全相同的对象；不同任务不共享可变键。只有仍持有有效 token/generation 的数据库事务会把本任务的版本化文件名写入 `cards.image_filename`；失租旧 worker 即使延迟上传最后完成，也只会写入它自己的旧任务键，不会覆盖后续任务已绑定的卡图。上传或插卡回滚时的对象清理失败仍会记录卡号和对象键。
