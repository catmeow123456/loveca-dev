...
> 当前状态：架构设计草案 v0.7；不包含具体数据库 schema、接口契约或代码改造步骤
> 最后更新：2026-06-18
...
- `Debug Replay Bundle`：管理员专用的临时导出 / 导入回放包，用于在正式持久化前验证记录格式、回放投影和问题复现；它不是普通用户历史对局记录。
...
需要特别区分：现有 `GameSession` 已经有运行时事件、审计、命令日志和内存快照，但它们不是长期历史记录。第一阶段的核心工程任务不是“把当前日志页面化”，而是增加一条持久记录边界，将运行中会话产生的权威事实稳定写入历史对局读模型。
...
2026-06-17 决策补充：正式落库前先推进 E0 管理员调试回放包。E0 可以直接从运行中会话导出带版本的 bundle，并由管理员只读导入查看；它不写入正式历史表、不开放普通用户、不承诺长期跨版本兼容。E0 的价值是尽早验证 recorder 输出、回放读取和卡效调试价值，等格式稳定后再迁移到 P0-P1 的正式持久化模型。

2026-06-18 收束：第一阶段时间线应以 `RecordFrame` 或等价账本模型为中心。每个 frame 分配 `timelineSeq`，命令、事件、决策、随机事实和 checkpoint 通过关联字段挂到 frame 上；不要让 `PublicEvent.seq`、`commandSeq`、`checkpointSeq` 各自成为互不对齐的回放顺序。checkpoint 和 debug bundle 的 payload envelope、序列化、复水、hash 校验和权限边界以 [checkpoint / bundle 序列化与复水契约](match-replay-serialization-contract.md) 为实现 review 基线。
...
- 第一阶段：记录式回放底座，先以 E0 管理员调试回放包验证格式，再按 P0-P3 拆分为记录创建与封存、时间线与玩家视角回放读取、决策说明与能力标记、回放体验增强与审计辅助。其中 P0-P1 是首个普通用户可交付 MVP，P2 是第一阶段复盘解释增强闭环。
...
本文档承接 [对局记录、复盘与可复现回放需求](match-replay-requirements.md)，回答以下问题：
...
本文档只描述架构方向、数据流和模型职责，不展开具体表结构、接口字段、迁移脚本或函数级实现。第一阶段的落地顺序、逻辑表、服务改造、API 和测试安排见 [第一阶段实施计划](match-replay-phase1-implementation-plan.md)。
...
- 运行中对局和房间仍由服务内存结构持有，历史对局不会在进程重启后保留。
- 数据库尚未有对局记录、时间线、检查点、决策日志或审计读取的长期模型。
- 现有快照以运行时恢复为主，且更接近按公共事件序号定位；尚未定义历史回放所需的独立检查点身份、保留、压缩和权限边界。
- 当前锁卡后的运行时卡组信息不足以直接成为历史卡组快照；历史快照还需要卡牌展示摘要、卡牌数据版本、合法性结论和锁定时间等输入。
...
因此，第一阶段应新增“记录边界”和“回放读取模型”两个稳定架构边界；现有 `GameSession`、投影和事件模型作为输入来源，而不是直接承担历史查询和长期存储职责。
...
2. P1：时间线与玩家视角回放读取
   持续追加 `Timeline Entry`、命令、公共事件、私密事件、审计摘要、随机结果摘要和检查点；历史详情页可以列出关键节点；普通玩家读取路径返回 `PlayerViewState + timeline explanation` 或等价对象，不返回权威检查点后交给前端隐藏。P1 首版可采用每个已接受命令后保存 authority checkpoint 的简单策略，但 schema 和 envelope 必须预留压缩、采样或关键节点 checkpoint 策略；普通玩家 timeline 需要独立过滤，不能复用管理员调试 timeline 摘要。
...
4. P3：回放体验增强与审计辅助
   完善回放 UI 的关键节点索引、事件说明、对局结束入口、部分记录提示和开发/审计导出。该阶段可以增强产品体验和排障能力，但不改变 P0-P2 的权限边界。

E0 达成后只代表管理员调试回放可用，不代表用户历史记录已上线。P0-P1 达成后即可交付第一版用户可用回放；P2-P3 达成后，第一阶段的复盘解释能力才算完整。具体实施拆分见 [第一阶段实施计划](match-replay-phase1-implementation-plan.md)。
...
- 手动处理与自由拖拽原因设计文档；
- 回放检查点粒度与保留策略设计文档；
- [checkpoint / bundle 序列化与复水契约](match-replay-serialization-contract.md)；
- 随机与洗牌记录设计文档；
- [第一阶段实施计划](match-replay-phase1-implementation-plan.md)。
