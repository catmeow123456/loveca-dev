...
- 保留并强化规则、联机与管理观测目标描述，满足“需求文档不写实现代码/实现细节”的规范
- 将易引发误解的“强实现承诺”改为需求边界描述（如配置化邮箱能力、关键窗口撤销、联机生命周期）
- 将流程描述修正为与当前规则引擎一致的高层流程（包含初始能量放置）
- 将正式联机房间与管理员房间监控纳入当前产品范围，移除“实时网络对战未纳入交付范围”的过时描述
