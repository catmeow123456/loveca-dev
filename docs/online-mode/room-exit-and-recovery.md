...
- 若权威对局已经进入 `GAME_END`，离开者的真人对局占用立即释放；双方都离开时立即清理旧房间运行态，不等待断线宽限；
- 对手仍在房间时，必须能从房间状态看到成员已离开；成员重新进入后，同一状态恢复为在线；
...
“认输”是进行中真人对局的独立终局操作，不等同于“退出房间”。

- 仅参与者可在未结束的 `IN_GAME` 对局中发起；普通联机房间和公共牌桌房间使用同一服务端权威命令；
- 提交前明确确认“认输后本局立即结束，无法恢复。”；
- 成功后权威状态以 `OPPONENT_SURRENDER` 结束本局，认输方显示“你已认输，本局结束。”，对手显示“本局获胜 / 对方已认输。”；
- 对局记录标记为 `SURRENDERED`，不允许以退出、刷新或重新进入恢复本局；
- 赛后“离开对局”才释放当前房间占用并返回主页，不能把暂时退出房间误作认输。
...
| 公共牌桌开局猜拳                    | 保留配对房间和房间号             | 退出房间；保留短暂恢复资格                           | 主页或公共牌桌的“返回房间”                             |
| 正式联机或公共牌桌的进行中对局      | 保留房间号，稍后重新同步玩家视角 | 退出房间保留恢复资格；认输则立即结束且不可恢复       | 退出后可从主页恢复；认输后离开对局可开始下一局         |
...
- 已进入公共牌桌开局后，退出房间与普通在线房间相同，按成员存活与房间生命周期决定是否仍可恢复。
...
4. 开局猜拳页同时提供“返回主页”和“退出房间”，且文案说明恢复语义。
5. 点击“返回主页”不会调用离开接口，不会清除房间恢复凭据。
6. 返回主页后可以从主页进入原房间。
7. 从公共牌桌进入已匹配房间时，会恢复客户端房间凭据。
8. 公共牌桌开局中退出房间后，对手能看到成员暂时不在房间；成员恢复后，对手能看到其重新在线。
9. 同一个匹配结果不会在玩家返回主页后再次强制跳转。
10. 新的公共牌桌匹配仍会自动进入一次。
11. 恢复请求被服务端拒绝时，页面显示明确错误并允许清理失效入口。
12. 桌面和移动端都能看见上述关键操作，键盘焦点与禁用状态清晰。
13. 进行中普通房间与公共牌桌房间都能认输；双方结果、公开事件与 `SURRENDERED` 战绩状态一致。
