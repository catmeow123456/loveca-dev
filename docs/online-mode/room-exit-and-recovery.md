...
未结束对局中的“返回大厅”只改变当前客户端页面，不发送服务端离开命令。赛季排位的进行中对局必须先提示：返回后可在 1 分钟内恢复，超时未返回会判负。权威对局已经结束时，“返回大厅”会先执行服务端离开命令、释放本人的真人对局占用，再完成导航；已结束对局没有继续恢复的必要。

必须满足：

- 未结束时保留当前房间号恢复凭据；已结束时成功释放后清除；
...
- 认输方不保留房间恢复凭据；对手仍可从赛后界面返回大厅并释放自己的房间占用；不能把暂时返回大厅误作认输。
...
| 已结束对局                          | 先释放本人旧房间占用，再返回大厅 | 不再显示认输或独立“离开对局”               | 不保留继续对局入口                                     |
...
16. 对局结束后点击“返回大厅”会释放本人旧房间占用；双方均离开后旧运行态立即清理。
