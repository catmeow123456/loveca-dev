...
> 当前状态：正式联机基础闭环已实现；卡效自动化第一阶段已部分接入；正式联机运行期 snapshot / command response 已走 JSON-native DTO 热路径；历史记录、timeline、authority checkpoint、public/private event 明细和玩家视角只读回放已阶段性落地；实时传输、进程重启后恢复运行中对局、完整随机/决策覆盖、确定性重演和全卡池完整自动裁判仍为后续方向
> 最后更新：2026-06-22
...
- 特殊成员卡堆叠窗口：卡效或命令层显式 `asMemberBelow` 可将成员卡堆叠到特殊成员卡下方（`memberBelow`）；普通拖拽到成员槽位不隐式堆叠，堆叠成员卡仍可拖回手牌（信任玩家撤回误操作）。
