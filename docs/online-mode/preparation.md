...
> 当前状态：正式联机基础闭环已实现；卡效自动化第一阶段已部分接入；正式联机运行期 snapshot / command response 已走 JSON-native DTO 热路径；实时传输、事件/快照的长期持久化、重放恢复和全卡池完整自动裁判仍为后续方向
> 最后更新：2026-06-19
...
- 运行期玩家视角 snapshot 与正式联机响应 DTO 已落地；事件/快照的长期持久化和进程重启后的对局恢复尚未落地。
...
事件/快照的长期持久化、重放恢复和进程重启后的恢复属于后续增强，不属于当前已落地链路；这不否定当前已经落地的运行期 snapshot 轮询与 JSON-native 响应热路径。
...
3. 为公共世界变化补齐公共事件语义。
4. 在现有运行期 snapshot / JSON-native 响应基础上，补事件持久化、快照长期持久化、重放恢复和幂等。
