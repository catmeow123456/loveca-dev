...
> 先冻结两个卡组的 canonical hash 与至少八个基础验证单元，让认证窗口通过 typed 决策契约、服务端 deadline 和具备有界活性的保守确定性策略完整推进，再接入测试 seeded RNG 与随机合法机器人，以可复现方式反复完成 headless 整局并在卡死时输出明确诊断。

这一步完成后，可以确定：

- 规则覆盖是否足够。
- 现有命令校验能否支撑合法候选生成。
- 所有强制窗口能否生成经共源校验的合法 witness。
- 哪些前端交互仍没有机器语义。
- authority revision、按席位窗口、lease 和单局提交生命周期是否完整。
- 服务端 deadline 在无浏览器、撤销和恢复场景下是否完整。
- 测试规则随机事实能否完整复现。
- 保守确定性策略能否在模型完全不可用时继续产生规则进展或在冻结边界内安全结束认证对局。
- 后续模型只需要解决策略质量，而不需要补规则和流程缺口。

在该闭环稳定前，Prompt、模型选型、AI Persona 美术、公共匹配和管理后台都不应成为主施工方向。
