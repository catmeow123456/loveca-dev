...
>
> 当前状态：Phase 0～4 已完成；Phase 4.5“语义决策上下文与服务端事实审计”已进入实施并为当前最高优先级，完成后再进入 Phase 5
>
> 日期：2026-08-02
>
...
- 是否值得换手、保留哪个展开点、当前应追求哪张 LIVE、资源应留给哪个后续窗口，主要交给 LLM 结合规则、完整卡组卡文和当前局面用语言推理判断；内部固定 playbook 只服务模型失败后的保守 witness，不进入模型请求。
...
模型不需要向系统输出私有思维链。模型必须输出的只有结构化选择；简短取舍和下一步计划是可选、低信任的调试信息。可审计的事实依据由服务端从通过权威契约校验的选择派生，不再要求模型抄写事实编号。
...
- 将压缩规则和完整卡组知识放入 system 消息，将当前局面、合法决策和精选历史放入 user 消息。
...
- 要求战术输出只必须给出当前 typed selection；简短取舍与下一步计划可以省略，不请求私有思维链。
- 在选择提交前复用权威 typed contract validator，并从通过校验的选择派生对应的事实审计引用；不把模型自由文本当成规则事实。
- 校验模型输出是否满足当前决策契约。
- 记录版本、延迟、token、错误和降级结果。
- 连续两次 JSON/选择协议失败时，通过服务端专用追加入口发布当前步骤的去重系统提示，只为这一步使用保守选择；供应商不可用、超时或预算拒绝才将本局剩余时间固定为保守策略。

模型调用边界不负责游戏规则、状态写入或“这个动作是否最优”的价值判断。只有 JSON、typed selection 结构或当前契约选择不合法才会在同一有效 lease 内触发有限修复；可选说明缺失或不符合格式不能成为拒绝合法选择的理由。

首版局内聊天是玩家沟通和系统状态展示通道，不是 AI 策略输入。玩家消息不得进入模型上下文；玩家显示名称等确需呈现的用户可控文本必须作为明确标记的数据字段处理，不能拼入规则 Prompt、完整卡组知识或其他指令段。
...
- 提供不可登录的稳定系统参赛者身份。
- 绑定公开名称、头像、简介和风格。
- 绑定固定卡组版本、规则 Prompt、内部保守 playbook 和策略参数。
- 在开局时冻结本局使用的版本快照。
- 支持后续替换、评测和回滚。
...
2. 两次返回仍无法解析为当前 typed selection 时，发布一次仅针对当前步骤的去重 `SYSTEM_NOTICE`，立即用保守策略处理这一步；下一决策继续正常请求模型。
3. 供应商持续或不可重试故障、超时、熔断或预算拒绝表示服务本身不可用；此时发布一次整局降级 `SYSTEM_NOTICE`，并将本局剩余时间固定使用保守策略，不再逐窗口试探已确认不可用的服务。
...
聊天提示由服务端生成，不伪装成 AI 玩家发言，不展示供应商名称、密钥、内部错误堆栈或重试细节。协议格式失败不改写本局策略模式；只有供应商/传输/预算类失败才进入整局保守模式。开发分支不保留旧降级政策的 dual-read 或 fallback。
...
模型的 system 消息按卡号与张数列出本局 AI 的完整卡组，包括：

- 卡牌编号、名称、类型与张数。
- 卡牌效果文本。
- 成员费用、BLADE 与 HEART。
- LIVE 分数与必要 HEART。
- 作品、团体、小组和 BLADE Heart 等可用公开字段。

该知识不包含洗牌顺序或卡牌实体 ID，也不写“某张卡必须如何使用”的固定打法。内部固定 playbook 只用于模型失败后的保守 witness，不进入模型请求。
...
> 当前进展（2026-08-02）：Phase 2 上下文与 Phase 4 模型调用边界均已完成，Phase 4.5
> 前四批语义上下文后端切片已落地。
> `src/server/ai-battle/ai-observation.ts` 的 `ai-battle.observation/v3`。适配器的公开输入只有
...
> 可见的正面卡牌、LIVE 公开修正以及 contract-local candidate/action/option 与输入约束；v3 还在
> 卡牌当前正面可见时提供不含对象 ID/隐藏顺序的候选区域，并为 active effect 提供投影允许的来源卡、
...
> 场面、能力属于谁”仍分散在不同对象中。Phase 4.5 前四批切片已在不扩大可见信息的前提下补齐
> 主要阶段成员登场/换手、LIVE 设置、active effect 来源/可选费用/可见目标，以及当前状态高于历史的
> 新鲜度语义，并以停机升级替换旧模型协议。后续切片又覆盖站位编排、复合分组选择和 LIVE 结算；
> 后续切片进一步把规则收敛为事实约束，并为模型注入本局完整卡组知识；舞台状态会显示有效费用、BLADE、
> HEART 和下回合换手基础，自送休息室的起动能力会显示费用支付后的舞台人数、休息室变化和后续目标来源。不保留
> dual-read 或运行时 fallback。
...
语义构造器在服务端内部同时保留稳定事实标识和适合阅读的短句；两者必须由同一份投影事实产生，不能分别维护两套内容。送模投影只发送短句和当前选择所必需的决策内临时 ID，不发送 `factId` / `requiredFactIds`；模型选择通过权威契约校验后，服务端再从内部事实标识派生审计引用。
...
3. **卡牌与规则事实**：完整本方卡组、公开卡文、压缩规则和已实现能力归属。
4. **模型结论**：本次选择、简短取舍和下一步计划，不得反向覆盖前三层。
...
当前 `selected-history/v4` 使用按席位隔离的定长窗口，只在命令被权威层接受后，根据脱敏 observation
...
Phase 4.5 已停止把模型生成的取舍或计划作为后续事实历史的正文来源。语义历史同时标出席位、来源类别和“当前状态优先”边界；跳过可选时点时还会明确说明该次选择已经结束，不能留到以后再发动。若未来需要跨窗口保留计划，只允许保存单独标记为 `MODEL_PLAN_NOTE` 的结构化意图，并且引用当前可见对象/目标；它不能声明卡牌能力、区域状态或已经发生的结果。当前实现完全不回灌模型计划，优先保证事实链不被模型自己的叙述污染。

### 8.7 不可信文本边界

首版模型上下文不包含局内聊天消息。玩家昵称、房间展示文本或未来其他用户可控字符串即使对该座位可见，也必须作为带明确字段边界的数据传入，不得与规则 Prompt、完整卡组知识、输出 schema 说明或其他系统指令拼接。
...
- 客户端通过认证账号查询唯一尚未离开的 AI 对局并恢复共享桌面；浏览器本地存储不是运行态所有者，丢失本地编号时仍可由服务端账号索引找回当前内存局。
...
- 确定性、启发式和 LLM 决策占比。
- 模型超时、重试和降级率。
- 服务端事实审计覆盖率、能力归属/槽位/替换/资源后果回归失败率，以及模型可选说明的评测质量。
- 当前步骤协议降级提示、整局供应商降级提示、去重次数和本局保守策略切换率。
...
AI 对战测试的目标不是冻结每一层对象的全部字段，而是分别证明规则合法性、语义正确性、出站协议和真实运行链路。协议升级时大量无关行为测试一起修改，说明测试跨越了职责边界；不能通过删除覆盖来降低代价，而应让共同改动面与真实责任一致。

测试按以下边界组织：

| 层级               | 主要输入与职责                                               | 主要断言                                                            | 不应绑定                              |
| ------------------ | ------------------------------------------------------------ | ------------------------------------------------------------------- | ------------------------------------- |
| 领域场景           | 权威状态、玩家投影、typed decision contract 和必要卡组       | 场景可达、候选合法、规则不变量成立                                  | model envelope、Prompt 文案、协议版本 |
| 语义行为           | allowlist observation、精选历史与合法候选                    | 来源、目标、支付、替换/保留、动作后资源及通俗正文正确               | 完整 system/user 序列化和无关字段顺序 |
| 协议契约           | 生产请求构造器的最终 provider-neutral 输出                   | 当前版本、system/user 分层、完整卡组知识、输出 schema、隐藏信息边界 | 对局策略优劣和整局流程                |
...
`tests/helpers/ai-battle/` 已作为测试基建的统一入口，分别提供策略场景、observation、语义上下文、模型 envelope 和 fake model 五类 builder。测试只覆盖自身关心的少量 override，默认值从生产导出的 schema/版本常量取得，不在每个测试手写完整嵌套 payload。场景 builder 与协议 builder 保持分离，没有用一个“全能 fixture”重新制造隐性耦合；测试场景按游戏意图命名，不按当前协议版本命名。现有手写 observation 已迁入该入口，模型 provider/governance 单测使用当前 envelope builder，整局 runtime 使用按 typed selection 生成合法返回的通用 fake model；分组选择、数字输入和完整站位提交另有聚焦回归。
...
| 改动类型                  | 应主要修改                                      | 原则上不应修改                         |
| ------------------------- | ----------------------------------------------- | -------------------------------------- |
| observation/envelope 结构 | 集中版本清单、对应 builder、少量协议契约测试    | 大多数语义行为和整局测试               |
| 中文语义或逐动作后果      | 对应语义构造器与命名场景                        | SYSTEM 身份、故障治理、其他选择类型    |
| 决策层级或 fallback 路由  | policy matrix、目标场景、必要的 fake model 覆盖 | observation 隐藏信息快照和卡组知识契约 |
...
> 当前进展（2026-07-30）：**COMPLETE**。本阶段最初冻结
...
> `strategy-context/v1` 构造；Phase 4.5 已按停机原则将当前运行协议整体替换为后文列出的 v6/v5/v4
> 语义版本，不保留 dual-read。请求包含不可执行数据边界和严格 JSON Schema；出站检查拒绝
> match/player/authority object identity、聊天、权限及用户显示文本字段。当前返回值只要求一个
> typed selection；取舍和计划是可选、低信任文本，未知顶层字段会被丢弃，不允许自由文本命令或私有推理参与执行；解析后仍复用权威
...
> 语言推理质量已经达到公共匹配标准。2026-08-02 的真实对局复盘确认：模型可以提交完全合法的
...
- 接入结构化策略选择。
- 建立模型调用超时、有限重试和取消，并把协议失败接入当前步骤保守选择，把供应商/传输/预算失败接入阶段 1B 已有的整局保守策略切换。
...
- 正式 `OnlineMatchService` 集成测试证明 provider 等待不占按局锁、选择与命令帧原子记录、协议失败只降级当前步骤、供应商失败切换整局保守策略，以及使用结构化模型选择完成完整双边对局。
...
> 第二批进一步把当前状态按中文阶段/子阶段、双方舞台及下方卡、资源、公开/共享区域组织；历史逐条
> 标明行动席位或受影响席位、权威接受/可见投影来源，并声明不能覆盖当前局面。active effect 则显式输出
> 来源卡、控制方、当前可见区域、当前步骤、可选费用与“不支付”分支、可见目标当前区域；来源转入隐藏
> 区域时只保留投影已授权的公开编号快照，不恢复完整卡面或位置。该切片以
> `PL!HS-sd1-006-SD` 费用 15「安养寺姬芽」的 LIVE 开始可选支付，以及
> `PL!-sd1-002-SD` 费用 2「绚濑绘里」从休息室回收成员的目标选择作为固定语义回归。
> 第三批将站位编排改为完整的“成员→目标槽位”配对事实，要求每名当前舞台成员恰好出现一次、目标成员区
> 不重复，并明确其下方能量/成员卡随来源成员移动；可选站位与可选槽位的跳过也改为 typed contract 实际接受的
> `CONFIRM_EFFECT`，不再暗示空 `placements` 合法。复合分组选择会将全局数量与每组数量限制组织为同一份
> 完整提交，候选属于多组时明确同时计数。LIVE 结算则补充双方权威总分、分数修正、已确认席位、逐卡判定、
> 卡牌级分数/必要 HEART 修正，以及所选成功 LIVE 进入成功区、其余 LIVE 在收尾进入休息室的权威后果；
> `PL!SP-bp4-008-P` 费用 13「若菜四季」站位变换和
> `PL!-sd1-019-SD` 分数 1「START:DASH!!」成功 LIVE 选择已加入固定语义回归。
>
...
> workflow 也已按卡文修正为有合法目标时强制选择，支付后来源自身若符合条件也属于合法目标。特殊登场及可跳过的
> 卡牌、选项、槽位、站位选择统一交给模型判断，不再由通用策略先行拒绝。
>
> 同日 23:44 的对局暴露了输出边界的反向问题：模型已连续两次给出合法的换牌 typed selection，但因没有抄写通用 `decision.base` 事实编号而被旧输出协议拒绝，并将整局固定切换到保守策略。这个事故推动事实审计改为服务端从 selection 派生，模型说明不再决定选择成败，协议失败的影响限制在当前步骤。后续真实对局又暴露大量费用未使用、低费成员错误覆盖同费成员，以及可选检索被通用保守策略直接跳过；因此 v6/v5/v4 停机升级进一步加入完整卡组 system 知识、公开场面数值、逐登场动作后果和模型负责的可选战术窗口。
>
...
- 将权威事实、卡牌/规则事实、内部 fallback playbook 和模型结论严格分层；fallback playbook 不进入模型请求。
- 让模型只必须输出结构化选择；简短取舍和下一步计划是可选调试信息，不请求或保存私有思维链。
- 在提交前严格验证 typed selection，并由服务端从该选择派生能力归属、槽位/替换关系与资源后果的事实审计引用；不对合法策略选择做服务端价值否决。
...
2. 当前 AI 卡组的完整卡牌知识；固定 playbook 不展示，因为它不进入模型请求。
3. 当前权威局面事实与卡牌公开文本。
4. 当前合法动作及每个动作的语言化后果。
5. 只由权威可见事实生成的精选历史。
6. 输出 schema/格式约束，以及存在第二次尝试时的受限 repair 指令。
7. 模型返回的已解析结构化选择与可选取舍/计划，以及服务端从选择派生的事实审计引用，供与输入上下文对照。
...
- observation、语义上下文或模型 envelope 的结构调整只修改对应 builder 与边界契约；无关行为测试不再因重复版本字符串和手写完整 payload 被迫同步修改。
...
- Phase 4.5 的语义上下文、服务端事实审计和策略质量回归已经完成；不能只凭“动作合法、整局能结束”把 AI 放入真人候场。
- 真人优先和 AI 补位规则清晰。
...
| 事实审计   | 服务端从通过契约校验的 selection 派生当前动作事实；可选模型理由只用于调试/评测，不以服务端收益值或启发式排序否决合法选择        |
| 历史纯度   | 精选历史只把权威接受结果和可见投影差异作为事实；模型摘要/计划保持低可信标记或不回灌，不能成为后续状态真值                       |
| 变更隔离   | 场景、语义、协议和运行时测试使用分层 builder；完整 snapshot 只冻结真实出站边界，协议升级不连带修改无关行为测试                  |
...
- Phase 3 内部入口的首版生命周期政策为：禁用撤销和 `FREE`，重开由 SYSTEM 自动接受并创建新局，显式离开按真人认输处理；服务端按账号发现唯一尚未离开的对局，刷新、清空浏览器会话缓存或创建冲突后都续接当前内存 match。
...
> 从当前 allowlist observation 与 typed contract 生成逐动作的权威事实和语言化后果，让模型用这些事实比较取舍；模型只必须返回 typed selection，服务端走既有合法性与命令提交链路，并从已通过校验的选择派生事实审计引用。
...
当前已经依次覆盖主要阶段成员登场/换手、有效费用/BLADE/HEART、下回合换手基础、LIVE 设置、active effect 来源、可选费用、可见目标、历史新鲜度、站位编排、复合分组选择、LIVE 处理、自送成员即时费用语义，以及完整卡组 system 知识。11.5 节测试基建收口已经完成集中协议版本、分层 builder、通用 fake model 和生产出站序列化边界测试；现有行为测试不再各自维护完整 observation/envelope 或窗口类型分支。下一步扩展服务端从合法 selection 派生的能力归属、站位与资源后果审计覆盖，使用真实 provider v7 请求协议和更广真人抽样复盘收束质量门槛，并按暴露出的决策问题追加窄回归。Phase 0～4 的规则、合法性、调度和故障门槛全部保留，不重复建设。
