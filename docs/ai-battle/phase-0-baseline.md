...
模型连续协议失败后，当前步骤使用保守选择，后续步骤继续请求模型；供应商不可用、超时或预算拒绝才将本局剩余时间固定切换到保守策略。进入整局保守模式后，连续 3 个 AI 回合无规则进展、累计 256 个保守决策或降级持续 5 分钟任一先到时，由 SYSTEM 认输，并记录区别于玩家认输和基础设施崩溃的终局原因。
