...
LIVE 设置窗口另由现有规则查询生成 `input.liveSet`：本次已盖对象、实际已盖数、受卡效修正后的设置上限、剩余额度与确认补抽数。它不从 LIVE 区总张数猜测，也不固定为 3。`decisionBrief.liveSet` 集中显示这些数据、手牌成员实例与重复数量、舞台当前 HEART 和活跃 BLADE。模型按“表演目标 → 近期保留集合 → 周转 → 确认”核对，每个保留用途须有近期位置、支付或能力依据。摘要另列接下来两个自己回合恢复并常规补能后的预算，以及手牌对各舞台槽位的印刷费用差；这些是未计卡效的基线，不是未来合法登场报价。响应 schema 与提示先列简短 `tradeoff` 预算结论，再列 `selection`；确认需交代盖牌/补抽数和停止追加理由，不能把剩余额度留到确认补抽后。
