...
结束主要阶段候选列出其他合法动作及登场预算，提示同时比较发展、后续能力预算和保留现有资源。仍允许模型直接结束，不依据静态增益强制出牌、不增加二次审查调用；模型参数和选择协议保持原样。输入设计的历史依据见[绿莲补场取舍](reviews/2026-09-09-3a5522fa-73.md)；策略验收边界以[支持矩阵](support-matrix.md)为准。
