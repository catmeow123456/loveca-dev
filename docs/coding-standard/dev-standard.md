...
- 未登记卡效、未覆盖效果段和未接线复杂时点继续遵循“信任玩家 + 命令校验 + 审计式流程”的边界。
- 自动卡效步骤如果暂未具备完整 `GameEvent` / `PublicEvent` 语义，必须至少保持权威状态、玩家视图投影、sealed audit 和限制文档一致；当该步骤需要被回放、事件持久化、观战增量同步或其他自动能力监听消费时，必须先补齐标准事件。
...
- 代码评审应拦截不合理实现、弱类型、妥协降级或静默吞错。确需类型放宽、兼容 fallback 或吞掉异常时，必须有清晰边界、显式理由和可验证的错误处理路径。
