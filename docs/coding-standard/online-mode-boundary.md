...
- 如果第一阶段卡效 helper 暂时还没有稳定事件模型，必须至少保证权威状态、玩家视图投影、sealed audit 和测试覆盖正确，并在 `docs/current-limitations.md` 或卡效登记文档中标注该步骤仍是 snapshot/audit 过渡语义。
...
- 正式联机中的“免费登场”是规则自动化缺口的正式兜底，不是调试作弊开关。它应作为命令级 `freePlay` 偏好提交，并继续由服务端校验操作者、阶段窗口、卡种和目标。
