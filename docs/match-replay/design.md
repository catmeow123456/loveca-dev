...
- `Debug Replay Bundle`：管理员专用的临时导出 / 导入回放包，用于验证记录格式、回放投影和问题复现；它不是普通用户历史对局记录。当前普通用户历史记录已走正式持久化链路，debug bundle 仍只属于高权限调试材料。
...
- 运行中对局和房间仍由服务内存结构持有，进程重启后恢复运行中对局尚未落地；已封存或已写入的历史记录可以通过 replay read model 读取。
