...
- 平台管理员“数据维护”页已提供 10 天回放清理预览与显式确认执行，以及按赛季导出的匿名原始分析 ZIP；数据包中的卡组信息来自不依赖回放保留期的长期 `ranked_deck_observations`，不会读取或导出 checkpoint、完整对局记录、原始对局卡组快照、timeline、聊天或回放。该页面不调用 shell，也不把导出文件写入 API 容器。自动低峰调度与持久化操作审计仍是后续工作。日常任务只应做保留期逻辑清理和 `VACUUM (ANALYZE)`，不在网页中执行 `VACUUM FULL`。
