...
- 存储或导出：只保存 transport 后的 JSON-safe payload；authority checkpoint 正式格式统一压缩为 GZIP 后 base64 存入 envelope。
...
- `compressed`：是否压缩。
- `compression`：正式 authority checkpoint 为 `GZIP`；`NONE` 仅作为旧数据迁移输入格式存在。
- `encoding`：正式 authority checkpoint 为 `BASE64_JSON`；旧未压缩格式为 `JSON_VALUE`。
...
- `match_checkpoints.payload_compression` 必须与 `payloadEnvelope.compression` 一致，正式读取路径要求二者都为 `GZIP`。
...
5. 对 authority checkpoint 使用 gzip 压缩 stable JSON，并将压缩 buffer 以 base64 写入 payload。
...
5. 校验 checkpoint 表字段与 envelope 的 `compression` 一致；正式路径拒绝 `NONE/JSON_VALUE`。
6. 解压 payload。
7. JSON parse。
8. 使用 `fromTransport<T>(payload)` 复水运行时对象。
...
## 8.1 旧 Checkpoint 压缩迁移

旧 `NONE/JSON_VALUE` authority checkpoint 不进入正式读取路径。停机窗口内使用迁移脚本全量转写：

```bash
DATABASE_URL=postgresql://... pnpm exec tsx src/scripts/compress-match-replay-checkpoints.ts --dry-run --report=tmp/replay-checkpoint-compress-dry-run.json
DATABASE_URL=postgresql://... pnpm exec tsx src/scripts/compress-match-replay-checkpoints.ts --apply --yes --batch-size=100 --report=tmp/replay-checkpoint-compress-apply.json
```
