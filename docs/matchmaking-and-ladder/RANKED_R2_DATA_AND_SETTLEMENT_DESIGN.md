...
> 当前状态：名为 V1 的首个生产赛季已开放并使用 V2；V3 代码与迁移工具已实现，生产 V2→V3 迁移尚未执行
> 最后更新：2026-08-02
...
代码显式保留 V1/V2 的 `ratingScale=400` 以重建旧流水，新赛季默认 V3：初始 `1500 / RD 300` 和定级参数不变，`ratingScale=800`。活动 V2 赛季的 V3 迁移只在停机边界追加同结果 `REPLACEMENT / VOID`；旧事件可保留不同算法版本，但每个对局的最新指令必须全部为当前赛季算法，否则重放拒绝。`SHADOW_V2` 只用于复现报告，不能创建赛季或写入正式流水；正式版本只能来自服务端注册表。

## 8. 下一步

1. 按 [活动排位赛季 V2→V3 停机迁移](RANKED_V3_ACTIVE_SEASON_MIGRATION.md) 完成 dry-run、报告复核与停机演练；
2. 补齐生产告警渠道、迁移前后积分波动核验与更细的运营指标；
3. 取得独立生产授权后再执行 V1 赛季的 V2→V3 迁移，并持续验证异常更正、自动收口和封存。
