...
> 当前状态：首批服务端接口与精简管理前端已上线；当前赛季使用 V3；V4 已加入新赛季正式算法列表
> 最后更新：2026-08-08
...
赛季另存 `leaderboardMinimumMatchCount`。V1–V3 仍允许 1–100 并可在运营边界修改；V4 创建页默认并锁定为 5，服务端拒绝草稿、激活或进行中赛季上的不一致值。它不参与单局积分数学，V4 将其锁定是为了让定级完成和进入排行榜使用同一语义。
