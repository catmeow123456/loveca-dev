...
> 当前状态：赛季/对局管理和 V3/V4 参数修订全量回算已实现；当前赛季使用 V3；V4 为新赛季默认
> 最后更新：2026-08-09
...
- “对局处理”：默认分页查看全部赛季的排位对局，也可按赛季筛选或按用户名、显示名称、用户 ID 搜索；列表直接显示双方胜负、胜方和结果类型。等待计分对局可重试计分，已结算对局可预览后执行不计分/改判。
