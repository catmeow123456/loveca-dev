...
> 当前状态：首批服务端接口与精简管理前端已上线；V3 代码已实现，尚未执行活动赛季迁移
> 最后更新：2026-08-02
...
当前服务端注册表包含影子候选 `GLICKO1_PER_MATCH_SHADOW_V2`、既有正式 V1/V2 和新赛季默认 V3。V1/V2 的 `ratingScale=400` 保持不变；V3 保持 V2 的初始 `1500 / RD 300` 和定级参数，只将 `ratingScale` 调为 800。创建、编辑和激活
...
- Shadow 版本创建被拒绝，正式 V1/V2/V3 可以创建草稿，新建页面默认 V3；
- 赛季、对局处理和监控读取正常；
