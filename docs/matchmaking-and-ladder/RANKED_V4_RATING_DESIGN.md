...
正式 V4 新赛季的排行榜门槛必须与 `placementMatchCount` 一致，固定为 5 场。管理端默认填入并锁定该值，服务端在草稿创建、激活和普通进行中运营修改边界均再次校验。受保护的参数修订可以在生成新算法身份并全量回算时同时改变两者；V1–V3 的历史正式配置与参榜规则保持不变。
...
成长池是排位 `rating_config` 的版本化扩展：旧 V1–V3 JSON 不含该字段仍可正常读取，正式 V4 则必须携带它并默认 `enabled=true`。服务端明确禁止 V1–V3 携带成长池，也禁止覆盖已发布的 V4 身份和配置。赛季内确需调整时，只能通过[评分参数修订与全量回算](RANKED_RATING_PARAMETER_REVISION.md)创建带唯一身份的新不可变修订；V4 修订可关闭成长池，关闭后所有对局只计算纯 Glicko，但仍保存基准分、单局总调整上限、回收分配比例和过渡宽度，便于后续重新启用。修订不会改写本文发布的 V4 基准。软重置策略仍按既有草稿边界配置。
...
V4 只面向新赛季，不修改 V3 迁移工具、旧影子报告脚本或旧赛季冻结配置。原始 V4 结算只使用现有 `rating_config` JSONB 和双方 before/after 流水。后续的赛季内参数修订由 `0017_add_ranked_rating_revisions.sql` 单独增加修订审计表和赛季当前修订引用，不改变 V4 单局计算结构。
