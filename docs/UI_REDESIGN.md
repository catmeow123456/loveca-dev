...
> 文档类型: 历史/计划文档
> 适用范围: UI 重设计背景、主题方向、历史设计稿和遗留清理参考
...
- [前端 UI 开发规范](coding-standard/ui-standard.md)：现行 UI 编码约束、主题 token 使用方式和组件边界。
- `client/src/index.css`：当前 CSS 变量、全局样式和主题实现。
- `client/src/lib/theme.ts`：当前主题检测、持久化和切换逻辑。
- `client/src/components/`：当前页面与组件实现。

本文保留重设计的背景、设计原则和历史取舍。若实际 token、组件结构或页面实现发生变化，应优先更新 UI 规范或代码，不要求同步修订本文的历史设计稿。
...
- 滚动条：使用 `var(--scrollbar-thumb)` / `var(--scrollbar-track)`；`.cute-scrollbar` 当前仅作为 token-backed 兼容别名保留
- 全局字体：`var(--font-body)`
...
### 游戏面板（模态框：JudgmentPanel / ScoreConfirmModal / MulliganPanel）
...
- **统一容器**：`var(--bg-elevated)` + `var(--radius-xl)` + `var(--shadow-lg)` + 顶部 3px 渐变强调条
  ...
  - 分数确认面板（ScoreConfirmModal）：成功/确认语义的绿色强调条 `linear-gradient(90deg, var(--semantic-success), var(--heart-green))`
- **内部元素**：
...
**移除 `.cute-*` 和 `.deck-manager` 硬编码样式**，改用令牌系统；`.cute-scrollbar` 兼容类不得再包含硬编码颜色：
...
- **滚动条**：`var(--scrollbar-thumb)` / `var(--scrollbar-track)`；`.cute-scrollbar` 保持为兼容类即可
...
| 阶段 | 做什么 | 涉及文件 |
|------|--------|----------|
| **1. 基础令牌 + 主题机制** | 用上述 CSS 变量替换 `index.css` 中所有硬编码颜色；删除旧 `.cute-*` 组件样式和旧 `.zone`/`.card-back` 硬编码样式，`.cute-scrollbar` 仅保留 token-backed 兼容别名；实现 `data-theme` 切换逻辑；创建 `ThemeToggle` 组件 | `index.css`、新建 `ThemeToggle.tsx`、`App.tsx` |
...
| **4. 游戏面板** | PhaseIndicator、PhaseBanner、GameLog、JudgmentPanel、ScoreConfirmModal、MulliganPanel、CardDetailOverlay | `game/*.tsx` |
| **5. 卡组管理与编辑器** | 移除旧 `.cute-*` 组件样式；`.cute-scrollbar` 目前仅保留为 token-backed 兼容别名 | `deck/DeckManager.tsx`、`deck-editor/*.tsx`（14 个子组件） |
...
| `.deck-manager ::-webkit-scrollbar` | `index.css` | 替换为使用 `var(--scrollbar-*)` 的全局滚动条样式 |
| `.cute-scrollbar` | `index.css` | 当前仅保留 `scrollbar-color: var(--scrollbar-thumb) var(--scrollbar-track)` 兼容别名，不再包含粉紫硬编码 |
