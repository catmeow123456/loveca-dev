...
当前 `model-request-envelope/v6` 把模型输入分成两层：system 消息包含可信的 `compact-rules/v3`、输出契约和 `deck-knowledge/v1`；后者按卡号与张数列出 AI 本局完整卡组的编号、名称、类型、卡文、费用、BLADE、HEART、LIVE 分数与必要 HEART，但不暴露洗牌顺序或卡牌实体 ID。user 消息只包含 `model-strategy-context/v5` 的通俗中文局面、当前完整合法选择、逐动作后果和纯净精选历史，不再发送固定 playbook 或硬编码打法指令，也不直接发送原始 observation、selected history 或事实编号。主要阶段会显示成员登场前后的有效费用/BLADE/HEART、换手基础、资源变化和成员区锁定；特殊登场及可跳过的卡效选卡/选项/槽位/站位交由模型判断，保守结果只在模型失败时作为 witness 使用。机械步骤继续由确定性策略直接处理，战术选择才调用固定的服务端 DashScope `qwen3.7-flash` profile。`model-decision-output/v3` 只要求 typed `selection`；一句 `tradeoff` 与 `nextPlan` 均可省略，说明文字缺失、换行、过长或类型错误不会否决合法选择。服务端会从已通过 typed contract validator 的选择自动派生审计事实，并继续重验 decision lease、authority revision、当前步骤和同一决策契约。调用具备单实例并发/速率、单局请求/token/估算费用、12 秒超时、最多两次尝试与状态变化取消；连续协议格式失败只让当前步骤使用保守选择，下一步重新请求模型，供应商不可用、超时或预算拒绝才让整局切换到保守策略。记录只保留版本、哈希、结构化选择、服务端派生事实引用、可选单行取舍/计划、执行结果、延迟、token、估算费用和哈希化 provider request id，不保存完整上下文、完整原始响应、供应商错误、聊天或凭据。开发分支只读当前 v6 request / v5 model context / v4 semantic context / v3 output 协议，不保留旧协议 dual-read。
