...
- 名为 V1 的首个赛季已在生产开放，当前使用 `GLICKO1_PER_MATCH_V3` 计分；V2→V3 工具与 runbook 保留为历史审计资料。V4 只供未来新赛季使用，不改写当前赛季；生产告警渠道和完整运营指标看板仍需补齐。排位运行态仍复用当前进程内联机房间，API 服务重启后的进行中对局恢复能力受正式联机同一限制。
