...
- 名为 V1 的首个赛季已在生产开放，当前使用 `GLICKO1_PER_MATCH_V3` 计分；V2→V3 工具与 runbook 保留为历史审计资料。V4 是未来新赛季默认。V3/V4 赛季可在管理端用受限参数创建独立修订并全量回算，但只能在暂停匹配、运行态归零和完整冻结环境校验通过后应用，不是无感热切换。生产告警渠道和完整运营指标看板仍需补齐。排位运行态仍复用当前进程内联机房间，API 服务重启后的进行中对局恢复能力受正式联机同一限制。
