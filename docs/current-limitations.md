...
- 认证 v1 -> v2 切换保留可识别的旧 bcrypt 密码，并在首次成功登录时升级为当前 v2 格式；旧会话和未完成的一次性链接仍会失效。已标记重置或未知格式的凭据无法恢复原密码，迁移会阻断并要求先完成独立账号恢复。
...
当前正式联机已经具备基础闭环：房间创建/加入、云端卡组锁定、双方准备开始、开局猜拳与胜者决定先后手、服务端权威对局、轮询同步、主动认输、房间号观战跨重开等待与自动续看、离开/短暂恢复、局内文字与快捷表情通信和管理员房间观测。局内通信允许对局双方发送、已授权观战者只读，并支持刷新或短暂断线后的游标补拉。公共牌桌 Beta 已提供持久候场、FIFO 配对、双方确认、60 秒双方到场确认、封闭房间引导和跨页面候场状态；其对局复用正式联机运行链路。赛季排位首版已在代码中复用同一状态机，并增加独立赛季/竞技环境候场、权威结果积分结算、排行榜和管理员异常更正。

仍未落地或未完整闭环：

- WebSocket/SSE 等实时传输增强；当前正式联机使用短间隔 HTTP 轮询。
- 局内文字和快捷表情只保存在当前 `OnlineMatch` 进程内运行态中，不写入数据库、历史记录或回放；API 服务重启、接受重开或双方离开导致旧对局运行态销毁后，消息不可恢复。
- 公共牌桌已能通过短租约接管确认后停滞的房间创建，并在重试耗尽时按心跳恢复无过错
  候场票据；尚未完成进行中房间的跨进程恢复、开局到场超时后的无过错方自动回队、
  完整运营指标聚合和运行后台。服务进程重启后，进行中的公共牌桌房间仍不能保证续局。
- 名为 V1 的首个赛季已在生产开放，当前使用 `GLICKO1_PER_MATCH_V3` 计分；V2→V3 工具与 runbook 保留为历史审计资料。V4 是未来新赛季默认。V3/V4 赛季可在管理端用受限参数创建独立修订并全量回算，但只能在暂停匹配、运行态归零和完整冻结环境校验通过后应用，不是无感热切换。管理端已有基础运行健康、经营指标与玩家分布概览；生产外部告警、跨日趋势和更细结算延迟分桶仍需补齐。排位运行态仍复用当前进程内联机房间，API 服务重启后的进行中对局恢复能力受正式联机同一限制。
...
- 对局记录与回放仍处于阶段性闭环：已接入正式联机与服务端可记录对墙打的历史根记录、卡组快照、timeline、authority checkpoint、public/private event 明细、普通玩家历史列表/详情/timeline/checkpoint 投影读取、前端历史入口、历史 checkpoint 只读 GameBoard 桌面展示，以及部分语义化 decision record。完整回放默认只保留最近 10 天；过期已封存记录在停机清理后会删除时间线、回放节点、bundle 所需负载和 `match_deck_snapshots` 卡组明细，只保留对局、参与者和卡组来源元信息。排位对局另有不支持回放的长期精简主卡组观察：只保存赛季/对局/座位/玩家、观察时间、稳定构筑指纹和按基础卡号聚合的卡名、类型、张数及可选图片文件名，用于赛季 Top 30、覆盖率、管理员核查和未来匿名聚类；它独立于 10 天清理，常规回放清理不得删除。对墙打第一版会将对手自动流程压缩在玩家命令后的 checkpoint 中；对墙打运行态缺失时可从最新 authority checkpoint 和公共事件尾部恢复，但可能回退到最近保存点且撤销历史会重置。本地 / 离线对墙打仍不保存历史。正式联机进程重启后恢复运行中对局、完整随机记录、完整决策覆盖、自由拖拽/手动处理原因结构化、确定性重演、逐命令动画播放、公开分享回放和长期兼容策略仍未完整闭环。
- 全卡池完整自动裁判、声明式卡文解释器、自动能力全事件编排和完整回放语义。当前已接入的卡效自动化属于第一阶段白名单能力，覆盖范围以卡效登记册为准。

相关文档：

- [联机模式准备文档](online-mode/preparation.md)
- [房间返回、退出与恢复设计](online-mode/room-exit-and-recovery.md)
- [联机模式边界规范](online-mode/boundary-standard.md)
- [赛季排位首版当前状态](matchmaking-and-ladder/RANKED_INITIAL_IMPLEMENTATION.md)
- [赛季排位数据与结算设计](matchmaking-and-ladder/RANKED_R2_DATA_AND_SETTLEMENT_DESIGN.md)
- [评分参数修订与全量回算](matchmaking-and-ladder/RANKED_RATING_PARAMETER_REVISION.md)
- [活动排位赛季 V2→V3 停机迁移](matchmaking-and-ladder/RANKED_V3_ACTIVE_SEASON_MIGRATION.md)
...
AI 对战 Phase 0～4 已完成首版受控闭环。两个认证卡组锁定 canonical YAML 内容哈希与 Phase 0 认证版本；不可登录 SYSTEM 席位只有在 `ONLINE + AI_BATTLE + 单一完整认证 binding` 下才能进入服务端机器调度，并复用权威命令、玩家视角投影、聊天、历史记录与共享游戏桌。登录玩家可以从独立入口选择双方固定卡组与 AI 先后手；服务端按账号维护唯一尚未离开的 AI 对局索引，应用刷新、浏览器会话缓存被清空、重新进入 AI 对战页或创建请求撞到已有对局时，都能查询并返回同一内存局。浏览器不再保存 AI 对局生命周期索引。AI 桌面显示局内系统通知和终局结果。本模式禁用撤销与 `FREE`，终局可按原配置重开，离开按真人认输处理。

当前 `model-request-envelope/v7` 把模型输入分成两层：system 消息包含可信的 `compact-rules/v4`、输出契约和 `deck-knowledge/v1`；压缩规则明确 `[E]`、冒号前复合费用、完整支付、可选不发动与换手费用差额，完整卡组知识则按卡号与张数列出 AI 本局卡组的编号、名称、类型、卡文、费用、BLADE、HEART、LIVE 分数与必要 HEART，但不暴露洗牌顺序或卡牌实体 ID。user 消息只包含 `model-strategy-context/v6` 的通俗中文局面、服务端结构化战略目标、当前完整合法选择、逐动作后果和纯净精选历史，不再发送固定 playbook 或硬编码打法指令，也不直接发送原始 observation、selected history 或事实编号。主要阶段会显示成员登场前后的有效费用/BLADE/HEART、换手基础、资源变化和成员区锁定；特殊登场、可跳过的卡效选择以及存在多个合法组合的强制选卡交由模型判断，保守结果只在模型失败时作为 witness 使用。机械步骤继续由确定性策略直接处理，战术选择才调用固定的服务端 DashScope `qwen3.7-flash` profile。`model-decision-output/v3` 只要求 typed `selection`；一句 `tradeoff` 与 `nextPlan` 均可省略，说明文字缺失、换行、过长或类型错误不会否决合法选择。服务端会从已通过 typed contract validator 的选择自动派生审计事实，并继续重验 decision lease、authority revision、当前步骤和同一决策契约。调用具备单实例并发/速率、单局请求/token/估算费用、12 秒超时、最多两次尝试与状态变化取消；连续协议格式失败只让当前步骤使用保守选择，下一步重新请求模型，供应商不可用、超时或预算拒绝才让整局切换到保守策略。记录只保留版本、哈希、结构化选择、服务端派生事实引用、可选单行取舍/计划、执行结果、延迟、token、估算费用和哈希化 provider request id，不保存完整上下文、完整原始响应、供应商错误、聊天或凭据。开发分支只读当前 v7 request / v6 model context / v5 semantic context / v3 output 协议，不保留旧协议 dual-read。

开发环境设置 `AI_BATTLE_DEBUG_TRACE_ENABLED=1` 后，管理员可以在自己进入的 AI 测试对局中打开“上下文”检查器，按模型尝试查看实际生效的 provider-neutral system/user 消息、规则/完整卡组知识、当前事实、合法选择及后果、精选历史、输出契约、解析后的选择与可选取舍/计划，以及服务端派生的审计事实、延迟、token、费用和权威执行结果。普通参与者即使是该局玩家也不能读取；生产环境强制关闭。展示内容与真实调用共用同一序列化函数，但不包含 API key、provider request id 原值、原始无效响应或私有思维链。上下文只在当前服务进程内存中保留，不进入聊天、录像、数据库或长期审计，服务进程重启后不会恢复。

与开发上下文检查器分离的 AI 对战反思历史已经默认用于每场 AI 对局，并可在生产配置下工作。它按完成的 AI 决策保存当时已经脱敏的局面摘要、结构化战略目标、合法选择、模型或确定性策略显式交付的短取舍/后续计划、最终选择、模型调用摘要和权威接受/拒绝/过期结果；管理员在自己进入的 AI 对局中可随时用桌面的“历史”按钮导出带自动复盘摘要、决策速览和完整审计附录的 Markdown 中途快照。文档不会保存完整 system/user Prompt、原始响应、聊天、凭据、权威对象 ID 或私有思维链，也不会回灌为后续模型事实。首版历史仍属于当前 `OnlineMatch` 的有界内存运行态（异常长局最多保留最近 1024 次决策并在文档中标记截断）；服务重启、重开或离开并删除运行态后不可再导出，尚未形成独立持久化归档。
...
- 独立入口已经明确展示 Loveca AI 身份并进入普通对局记录；完整头像/简介、胜率、奖励及其他竞技统计仍未确定。
...
- 已实现卡效范围以 [卡效完成状态登记册](card-effect-reuse-audit/existing_module_map.md) 为准。`RULES` 已通过中央玩家命令政策拒绝普通手动区域移动、能量操作、成员换位及与当前阶段/pending 无关的命令；未登记卡、未覆盖效果段和未接线复杂时点不应写成当前自动执行能力。需要人工落实其规则结果时，必须显式进入 `FREE`，正式联机还需对方同意。
