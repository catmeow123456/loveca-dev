...
- 名为 V1 的首个赛季已在生产开放，当前仍使用 `GLICKO1_PER_MATCH_V2` 计分。`GLICKO1_PER_MATCH_V3` 代码和同赛季停机迁移工具已经实现，但生产 V2→V3 迁移尚未执行；生产告警渠道和完整运营指标看板仍需补齐。排位运行态仍复用当前进程内联机房间，API 服务重启后的进行中对局恢复能力受正式联机同一限制。
