...
AI 对战 Phase 0～4 已完成首版受控闭环。两个认证卡组锁定 canonical YAML 内容哈希与 Phase 0 认证版本；不可登录 SYSTEM 席位只有在 `ONLINE + AI_BATTLE + 单一完整认证 binding` 下才能进入服务端机器调度，并复用权威命令、玩家视角投影、聊天、历史记录与共享游戏桌。登录玩家可以从独立入口选择双方固定卡组与 AI 先后手，刷新续接同一内存局；AI 桌面显示局内系统通知和终局结果。本模式禁用撤销与 `FREE`，终局可按原配置重开，离开按真人认输处理。

模型只接收由 Phase 2 allowlist observation 与纯净精选历史派生的 `model-strategy-context/v1`，其中包含版本化规则/playbook、中文局面事实、当前合法选择和逐动作后果，不直接发送原始 observation 或 selected history。机械窗口继续由确定性策略直接处理，战术窗口才调用固定的服务端 DashScope `qwen3.7-flash` profile。模型返回只能表达 typed selection、事实引用、单行取舍与下一步计划；提交前会检查引用存在且覆盖所选方案的必要事实，并继续重验 decision lease、authority revision、窗口和同一决策契约。调用具备单实例并发/速率、单局请求/token/估算费用、12 秒超时、最多两次尝试与状态变化取消；确认失败后发送一次系统提示，并让本局固定切换到保守策略。记录只保留版本、哈希、结构化选择、事实引用、单行取舍/计划、执行结果、延迟、token、估算费用和哈希化 provider request id，不保存完整上下文、完整原始响应、供应商错误、聊天或凭据。

开发环境设置 `AI_BATTLE_DEBUG_TRACE_ENABLED=1` 后，管理员可以在自己进入的 AI 测试对局中打开“上下文”检查器，按模型尝试查看实际生效的 provider-neutral system/user 消息、规则/playbook、当前事实、合法选择及后果、精选历史、输出契约、严格解析后的选择/事实引用/取舍/计划，以及延迟、token、费用和权威执行结果。普通参与者即使是该局玩家也不能读取；生产环境强制关闭。展示内容与真实调用共用同一序列化函数，但不包含 API key、provider request id 原值、原始无效响应或私有思维链。上下文只在当前服务进程内存中保留，不进入聊天、录像、数据库或长期审计，服务进程重启后不会恢复。

仍未落地或未完整闭环：

- 当前只允许两套认证卡组；任意玩家卡组、全卡池完整自动裁判和未认证规则版本不能进入 AI 对战。
- Phase 4.5 第一批语义行动后果只覆盖主要阶段成员普通登场/换手与 LIVE 盖放/收回等基础路径；卡效来源、可选费用、目标选择和其他复杂窗口仍缺少更广固定语义回归。
- 当前事实自检只验证引用存在且覆盖所选方案声明的必要事实，不解析模型自由文本来裁决能力归属、槽位关系或资源结论，也不建设复杂收益值、后悔值或按卡牌编号维护的最优策略引擎；真实 provider 新协议评测和真人抽样复盘尚未完成。
- `selected-history/v3` 已从脱敏 observation 与权威接受后的结构化选择/可见投影差异派生，模型自由文本不再作为后续事实回灌；AI 对局运行态和这份精选历史仍只保存在单个服务进程内。
