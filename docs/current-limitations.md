...
- `basic-live-loop@1.1.6` 已通过服务端临时权威会话和共享游戏桌提供四章入口。教程会话、访问令牌映射、访客创建限流和活跃会话计数只保存在当前 API 进程内，不跨实例共享，进程重启后失效；页面刷新会返回章节选择，不恢复章节内部步骤。
- 当前自动化验证覆盖确定性场景、玩家投影、命令门禁、章节检查点和引导控制器；多视口、键盘、reduced-motion、卡图 fallback 与对手隐藏视角仍需按[教程需求](tutorial/requirements.md)完成人工或 E2E 验收。

## 平台整站维护部署边界

三态平台状态、独立公开快照协议、应用级维护/故障页、管理员恢复入口、数据库就绪检查与结构化状态日志已经在代码中落地。生产反向代理配置不在仓库中，正式启用前仍需完成以下部署与验收：

- 将持久目录中的 `site-status.json` 映射为同源 `/site-status.json`，并确认响应禁止浏览器、CDN 与代理缓存；该目录不能随 `client/dist` 切换。
- 首次部署新前端前先创建有效 `OPEN` 快照，否则客户端会按故障安全策略显示服务不可用。
- 在真实生产拓扑完成 API 停止、数据库未就绪、快照冲突、无痕窗口、已安装 PWA、移动端和日夜主题验证。
- 当前 `/api/ready` 覆盖数据库连接与必要表，不把 MinIO、邮件和其他非启动必需外部依赖作为开放门禁；未来若某依赖变成当前版本普通页面的硬前置，需要同步扩展就绪契约。

权威状态模型与操作顺序见 [平台状态、整站维护与公告](platform-operations/site-status-and-announcements.md)。
...
当前正式联机已经具备基础闭环：房间创建/加入、云端卡组锁定、双方准备开始、开局猜拳与胜者决定先后手、服务端权威对局、轮询同步、主动认输、房间号观战跨重开等待与自动续看、离开/短暂恢复、局内文字与快捷表情通信和管理员房间观测。局内通信允许对局双方发送、已授权观战者只读，并支持刷新或短暂断线后的游标补拉。公共牌桌 Beta 已提供持久候场、FIFO 配对、双方确认、60 秒双方到场确认、封闭房间引导和跨页面候场状态；其对局复用正式联机运行链路。赛季排位首版已在代码中复用同一状态机，并增加独立赛季/竞技环境候场、权威结果积分结算、排行榜和管理员异常更正。轮换主题牌桌第一版进一步复用该状态机，以独立活动环境、服务端预组和冻结组合完成非计分候场、匹配后的 `X 选 1` 候选选择、正式房间引导及开局前无过错回队。
...
- 轮换主题牌桌只达到开发/内测基线：尚未冻结首期真实预组与实测组合，缺少完整等待/确认/组合胜率和异常率指标、选用分布告警、主题身份历史详情、进行中房间跨进程恢复与双浏览器验收；完成这些前置前不得开放生产活动。
...
- [轮换主题牌桌策划与设计](matchmaking-and-ladder/ROTATING_THEME_TABLE_DESIGN.md)
