...
当前正式联机已经具备基础闭环：房间创建/加入、云端卡组锁定、双方准备开始、开局猜拳与胜者决定先后手、服务端权威对局、轮询同步、主动认输、房间号观战跨重开等待与自动续看、离开/短暂恢复、局内文字聊天和管理员房间观测。局内聊天允许对局双方发送、已授权观战者只读，并支持刷新或短暂断线后的游标补拉。公共牌桌 Beta 已提供持久候场、FIFO 配对、双方确认、60 秒双方到场确认、封闭房间引导和跨页面候场状态；其对局复用正式联机运行链路。赛季排位首版已在代码中复用同一状态机，并增加独立赛季/竞技环境候场、权威结果积分结算、排行榜和管理员异常更正。

仍未落地或未完整闭环：

- WebSocket/SSE 等实时传输增强；当前正式联机使用短间隔 HTTP 轮询。
- 局内聊天只保存在当前 `OnlineMatch` 进程内运行态中，不写入数据库、历史记录或回放；API 服务重启、接受重开或双方离开导致旧对局运行态销毁后，消息不可恢复。
- 公共牌桌已能通过短租约接管确认后停滞的房间创建，并在重试耗尽时按心跳恢复无过错
  候场票据；尚未完成进行中房间的跨进程恢复、开局到场超时后的无过错方自动回队、
  完整运营指标聚合和运行后台。服务进程重启后，进行中的公共牌桌房间仍不能保证续局。
- 赛季排位尚未执行预发布或生产数据库迁移，也未开放首季；生产告警渠道、完整运营指标看板、首季配置和 POC 判断口径仍需在发布前冻结。排位运行态仍复用当前进程内联机房间，API 服务重启后的进行中对局恢复能力受正式联机同一限制。
...
- 对局记录与回放仍处于阶段性闭环：已接入正式联机与服务端可记录对墙打的历史根记录、卡组快照、timeline、authority checkpoint、public/private event 明细、普通玩家历史列表/详情/timeline/checkpoint 投影读取、前端历史入口、历史 checkpoint 只读 GameBoard 桌面展示，以及部分语义化 decision record。完整回放默认只保留最近 10 天；过期已封存记录在停机清理后仅保留对局、参与者和卡组来源元信息，不能再读取时间线、回放节点或导出 bundle。对墙打第一版会将对手自动流程压缩在玩家命令后的 checkpoint 中；对墙打运行态缺失时可从最新 authority checkpoint 和公共事件尾部恢复，但可能回退到最近保存点且撤销历史会重置。本地 / 离线对墙打仍不保存历史。正式联机进程重启后恢复运行中对局、完整随机记录、完整决策覆盖、自由拖拽/手动处理原因结构化、确定性重演、逐命令动画播放、公开分享回放和长期兼容策略仍未完整闭环。
