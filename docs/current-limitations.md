...
- 轮换主题牌桌只达到开发/内测基线：尚未冻结首期真实预组与实测组合，缺少完整等待/确认/组合胜率和异常率指标、曝光偏差告警、主题身份历史详情、进行中房间跨进程恢复与双浏览器验收；完成这些前置前不得开放生产活动。
