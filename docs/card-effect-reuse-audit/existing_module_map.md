...
| `PL!-sd1-008-SD` | `T03,T07,F06,E01` | 每回合一次已统一为来源卡实例级限制；支付 `[E][E]` 走 effect-costs；顶 10 入休息室走 look-top top-deck move。 |
...
| `PL!HS-bp6-017-N` 费用 11「日野下花帆」 | `T06,S08,C01,F07,F08,F09` | AUTO：此成员从舞台放置入休息室时，可以弃 1 手牌；如此做时，从休息室将 LIVE 卡和成员卡至多各 1 张加入手牌。复用 `ON_LEAVE_STAGE` 入队、弃手费用、`WAITING_ROOM -> HAND` 移动；分组上限由 runner 校验。测试：`tests/unit/card-effect-classification.test.ts`、`tests/integration/sample-card-effect-runner.test.ts`。 |
| `PL!HS-pb1-009-R` 费用 15「日野下花帆」 | `T06,T07,B01,T02,F02` | AUTO 第一段：中央时每当己方「莲之空」成员登场至自己舞台，LIVE 结束时为止获得 BLADE x2；实例级每回合 2 次已接通。第二段：LIVE 开始时用成员有效 BLADE helper 判断印刷 BLADE + 同来源 modifier 是否大于等于 8，满足时复用抽 2 弃 1。手动从顺序选择窗口点选第一段时会进入通用 confirm-only 确认壳；顺序发动不弹该壳。测试：`tests/unit/live-modifiers.test.ts`、`tests/unit/card-effect-classification.test.ts`、`tests/integration/sample-card-effect-runner.test.ts`。 |
| `PL!HS-bp6-004-R` 费用 13「百生 吟子」 | `T01,T02,S01,S03,C01,B01,X03,X05,X06` | 登场 / LIVE 开始：选择对方舞台费用小于等于 9 的 1 名成员变为待机状态，复用 `stage-targets.ts` 舞台成员目标 helper、`card-selectors.ts` 组合 selector 与 `setMemberOrientation`。LIVE 开始：可弃 1 手牌，获得 BLADE；弃置姓名归一化后为「百生吟子」的成员时获得 BLADE +2，复用 `cardNameIs`、可选弃手步骤与 `addLiveModifier`。同一来源卡两条 LIVE 开始能力会在顺序选择窗口使用具体效果 option 区分。测试：`tests/unit/card-effect-classification.test.ts`、`tests/unit/card-selectors.test.ts`、`tests/integration/sample-card-effect-runner.test.ts`。 |
...
| Trigger enqueue functions in `src/application/card-effect-runner.ts` | `T01,T02,T04,T06,S08` | 支持登场、Live 开始、Live 成功、离场 AUTO、舞台成员监听登场 AUTO 与同一时点/同事件队列；登场与舞台成员 LIVE 开始会记录来源槽位，能力可通过 `requiredSourceSlots` 统一过滤左/中/右区域条件。手动从顺序选择窗口点选无输入 pending ability 时，可进入通用 confirm-only active effect；顺序发动仍自动连续处理。同一窗口若存在重复来源卡 ID，会使用 `selectableOptions` 按具体 pending ability 区分，避免同源双效果无法选择。离场 AUTO 已由 `PL!HS-bp2-012-N` 费用 5「乙宗 梢」与 `PL!HS-bp6-017-N` 费用 11「日野下花帆」验证，登场监听 AUTO 已由 `PL!HS-pb1-009-R` 费用 15「日野下花帆」验证，同源双 LIVE 开始已由 `PL!HS-bp6-004-R` 费用 13「百生 吟子」验证；尚不是完整 `GameEvent -> trigger matcher`。 | 001,003,004,006,007,009,011,012,015,016,019,022,Karin,PR-004,Shiki,`PL!HS-bp2-012-N`,`PL!HS-bp6-017-N`,`PL!HS-pb1-009-R`,`PL!HS-bp6-004-R` |
| `src/application/effects/card-selectors.ts` | `X04,X05,X06` | 提供 `typeIs`、`groupIs`、`costLte`、`cardNameIs`、`and/or/not` 等最小 selector。尚未覆盖 cardCode 集合、舞台状态、成功区分数等复杂条件。 | 001,002,003,004,005,015,Karin,`PL!HS-bp6-004-R` |
| `src/application/effects/stage-targets.ts` | `S03,X06` | 提供按 `playerId + CardSelector` 扫描左/中/右成员区的目标候选 helper；当前用于对手舞台低费成员目标，后续可扩成完整目标选择 config。 | `PL!HS-bp6-004-R` 费用 13「百生 吟子」 |
| `src/application/effects/zone-selection.ts` | `F07,F08,F09` | 提供 `ZoneCardSelectionConfig`、`createWaitingRoomToHandEffectState`、`moveSelectedCardsFromZone`；当前主力是 `WAITING_ROOM -> HAND`，已支持单选与 `maxCount` 多选配置。分组上限（如 LIVE/成员各至多 1 张）仍由具体 runner 校验，后续可抽 grouped selection config。 | 001,002,003,005,`PL!-pb1-019-N`,`PL!-bp4-003-P`,`PL!HS-bp2-002-P`,`PL!HS-bp6-017-N` |
| `src/application/effects/effect-costs.ts` | `C01,C02,C03,C04,E01` | 提供 `EffectCostDefinition`、弃手选择费用、即时横置能量、自送休息室。移动会记录 action，但还没有标准 `GameEvent`。 | 002,003 Live-start,005,008,011,012,015,016,`PL!-pb1-019-N`,`PL!-bp4-003-P`,`PL!HS-bp6-017-N`,`PL!HS-bp6-004-R` |
...
| `src/application/effects/draw.ts` + draw-discard shell in runner | `F01,F02` | `drawCardsFromMainDeckToHand` 提供卡效步骤抽牌；`startDrawThenDiscardOneEffect` / `finishDrawThenDiscardOneEffect` 组合抽 N 后弃 1。暂不接管开局/阶段/Live 判定抽牌，也不改变当前刷新语义。 | 007,Shiki,`PL!HS-bp1-006-P`,`PL!HS-pb1-009-R` |
...
| `src/application/effects/member-state.ts` | `S01,S02,S05,S07` | 提供 `setMemberOrientation` / `setMembersOrientation`、`moveMemberBetweenSlots` 与 `playMembersFromWaitingRoomToEmptySlots`。普通规则横置/拖拽仍在 GameSession/action handler；当前 S07 卡效登场只进空槽。 | Karin position change, Shiki live-start position change, `PL!SP-bp5-003-AR` 费用 17「岚 千砂都」, `PL!N-pb1-008-P+` 费用 17「艾玛·维尔德」, `PL!S-bp2-006-P` 费用 11「津岛善子」, `PL!HS-bp6-004-R` 费用 13「百生 吟子」 |
| `src/domain/rules/live-modifiers.ts` | `T05,B01,B03,B05,B07,B08` | `collectLiveModifiers` 是 Live 判定读路径；`addLiveModifier` / `replaceLiveModifier` 是临时 Live 修正主写入路径；legacy maps 作为兼容投影。 | 001,003,009,022,Live judgment,`PL!HS-pb1-009-R`,`PL!HS-bp6-004-R` |
...
| Active effect UI shape in `src/domain/entities/game.ts` and `client/src/components/game/GameBoard.tsx` | `X03,F05,B03,S05` | 支持 card selection、ordered multi-select、slot selection、option selection；这是 UI/状态形状，还不是 resolver DSL。同源多 pending ability 的顺序选择已能复用 option selection。 | 003 Heart choice,019 ordered top,Karin/Shiki position change,`PL!N-pb1-008-P+` 费用 17「艾玛·维尔德」,`PL!HS-bp6-004-R` 费用 13「百生 吟子」 |
...
- `PL!HS-bp2-012-N` 费用 5「乙宗 梢」与 `PL!HS-bp6-017-N` 费用 11「日野下花帆」已完成前两条 `ON_LEAVE_STAGE` AUTO；look-top / 弃手后分组回收解析仍在 runner 串联，完整 declarative workflow 与完整 `GameEvent` 层后续再抽。
- `PL!HS-pb1-009-R` 费用 15「日野下花帆」已完成舞台成员监听 `ON_ENTER_STAGE` 的 AUTO 第一段与实例级每回合 2 次，并完成 LIVE 开始 BLADE >= 8 抽2弃1段；完整条件 DSL 仍后续再抽。
- `PL!HS-bp6-004-R` 费用 13「百生 吟子」已完成对手低费成员待机、指定姓名弃手加 BLADE、同源双 LIVE 开始顺序选择区分；舞台目标 helper 与 `cardNameIs` 已抽出，后续可继续把目标选择 active effect 配置化。
- Standard movement/events for broader AUTO listeners are still incomplete; Stage 1O/1P currently covers leave-stage and enter-stage proving paths only.
