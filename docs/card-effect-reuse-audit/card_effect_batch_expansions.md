...
| `PL!HS-bp5-001` | 费用 11「日野下花帆」 | 已实现；登场将顶 4 放入休息室并公开展示，展示结束后若其中存在 LIVE，LIVE 结束时为止获得 BLADE +2 |
...
| `PL!HS-PR-019` | 费用 2「百生吟子」 | 已实现；登场将顶 3 放入休息室并通过 Public Reveal Dwell 公开展示，展示结束后若 3 张全为持有绿色 Heart 的成员则获得绿色 Heart。`PR / RM` 由基础编号匹配同步。 |
| `PL!HS-bp5-001` | 费用 11「日野下花帆」 | 已实现；登场将顶 4 放入休息室并公开展示，展示结束后按是否存在 LIVE 获得 BLADE +2；起动支付 2 能量公开手牌 LIVE，并从休息室回收同名 LIVE。`AR / P / R+ / SEC` 由基础编号匹配同步。 |
...
- 检视后公开的卡继续由 `inspectionZone` 持有；direct mill 则先完成真实区域移动，再展示本次 `movedCardIds`。两类隐藏卡刚变为双方公开后，都通过 shared Public Reveal Dwell 保证阅读时间，再自动进入最终结算或下一真实交互。
- 隐藏区候选在确定公开前使用 `selectableCardVisibility: AWAITING_PLAYER_ONLY`，对手视角不显示候选区和候选数量；公开手牌后只将本次明确公开的 cardIds 交给 Public Reveal Dwell 向双方展示。
