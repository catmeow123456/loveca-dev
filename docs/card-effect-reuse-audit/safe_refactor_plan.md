...
> 现行公开展示说明：本文保留的“点击继续处理”“确认窗口”等用语是历史实施快照，不代表当前 UI/运行时。隐藏卡刚变为双方公开后的现行机制是 `runtime/public-reveal-dwell.ts` 的无按钮权威定时展示；休息室/声援公开区具体选卡仍使用独立的 public-card-selection confirmation。
