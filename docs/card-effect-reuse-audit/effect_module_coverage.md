...
| Trigger enqueue functions in `src/application/card-effect-runner.ts` | `T01,T02,T04,T06,S08,E06` | 支持登场、LIVE 开始、LIVE 成功、自己进行声援时、离场 AUTO、舞台成员监听登场 AUTO 与同一时点/同事件队列。登场与舞台成员 LIVE 开始会记录来源槽位，能力可通过 `requiredSourceSlots` 统一过滤左/中/右区域条件。LIVE 成功已支持成功 LIVE 卡来源与表演玩家舞台成员来源；`ON_CHEER` 当前扫描表演玩家 LIVE 区来源；登场 AUTO 优先消费 `EnterStageEvent`；离场 AUTO 优先消费 `LeaveStageEvent`，可携带换上成员 `replacingCardId` 做 relay 来源条件。 | `PL!HS-bp2-012` 费用 5「乙宗 梢」、`PL!HS-bp6-017` 费用 11「日野下花帆」、`PL!HS-sd1-001` 费用 9「日野下花帆」、`PL!HS-pb1-009` 费用 15「日野下花帆」、`PL!HS-bp6-001` 费用 4「日野下花帆」、`PL!HS-cl1-009` 分数 1「水彩世界」、`PL!HS-bp6-027` 分数 5「月夜見海月」 |
...
| `src/application/effects/member-state.ts` | `S01,S02,S05,S07,S09` | 提供 `setMemberOrientation` / `setMembersOrientation`、`moveMemberBetweenSlots` 与 `playMembersFromWaitingRoomToEmptySlots`。方向改变会写入 `ON_MEMBER_STATE_CHANGED` eventLog；成员区空槽移动/交换会写入 `ON_MEMBER_SLOT_MOVED` eventLog，并继续记录 `positionMovedThisTurn`；卡效从休息室登场会写入 `ON_ENTER_STAGE` eventLog。普通 `MOVE_MEMBER_TO_SLOT` 也写入同族事件；`enqueueTriggeredCardEffects` 已消费成员槽位移动与登场事件。 | `PL!N-pb1-004` 费用 11「朝香果林」、`PL!SP-bp4-008` 费用 13「若菜四季」、`PL!SP-bp4-011` 费用 7「鬼冢冬毬」、`PL!SP-bp5-003` 费用 17「岚 千砂都」、`PL!N-pb1-008` 费用 17「艾玛·维尔德」、`PL!S-bp2-006` 费用 11「津岛善子」、`PL!HS-bp1-002` 费用 11「村野沙耶香」、`PL!HS-bp6-004` 费用 13「百生 吟子」 |
