...
| `src/application/effects/conditions.ts` | `X01,L01,L02,X13` | 第一版纯函数 condition/query helper：区域卡牌计数、按 selector 计数与阈值判断、按 selector 返回 cardIds、成功 LIVE 数、舞台成员数、舞台成员存在性、来源以外其他舞台成员、LIVE 区排除来源卡计数、来源成员有效 BLADE 阈值查询。当前不做 AST、不做声明式 steps，倍率公式与 workflow 仍留在 runner。 | `PL!-sd1-001` 费用 7「高坂穗乃果」、`PL!-sd1-009` 费用 11「矢泽妮可」、`PL!-sd1-022` 分数 4「僕らは今のなかで」、`PL!HS-bp5-019` 分数 6「花结」、`PL!HS-bp2-022` 分数 2「アオクハルカ」、`PL!HS-pb1-009` 费用 15「日野下花帆」、`PL!HS-pb1-020` 费用 9「百生吟子」、`PL!HS-sd1-006` 费用 15「安养寺姬芽」、`PL!HS-bp6-001` 费用 4「日野下花帆」、`PL!HS-bp6-031` 分数 8「ファンファーレ！！！」、`PL!HS-bp1-006` 费用 11「藤岛 慈」 |
