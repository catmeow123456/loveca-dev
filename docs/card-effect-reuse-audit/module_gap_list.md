...
| `X01,L01,L02,X13` | `src/application/effects/conditions.ts` + `card-selectors.ts` | 第一版纯 query helper 已起步，提供区域计数、selector 计数/阈值、按 selector 返回 cardIds、成功 LIVE 数、舞台成员数/存在性、其他舞台成员、LIVE 区排除来源卡计数、来源 BLADE 阈值等。当前只替换低风险内联计数，不做 condition AST、typed formula builder 或 declarative steps。 |
