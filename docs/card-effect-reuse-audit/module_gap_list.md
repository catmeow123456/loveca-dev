...
|---|---|---|
| `F07,F08,F09` | `src/application/effects/zone-selection.ts` + `runtime/actions.ts` + waiting-room recovery workflows | 当前 `WAITING_ROOM -> HAND` 底层移动已由 `recoverCardsFromWaitingRoomToHandForPlayer` 收束；`shuffleWaitingRoomCardsToDeckBottomForPlayer` 已收束 caller 指定休息室卡洗切后放主卡组底的底层移动，当前用于 `PL!HS-pb1-012` 与 `PL!HS-bp6-031` 的成员洗回段，selector、数量统计、奖励与后续回收仍由 caller 负责。纯 pending 回收、纯自送回收、支付能量回收、`PL!-bp4-002` 弃手费用回收已迁出 runner，分别落在 `workflows/shared/waiting-room-to-hand.ts`、`self-sacrifice-waiting-room-to-hand.ts`、`pay-energy-waiting-room-to-hand.ts`、`discard-cost-waiting-room-to-hand.ts`。`PL!HS-sd1-006` 登场活跃能量后回收已迁到 `workflows/cards/hs-sd1-006-hime.ts`，`PL!-PR-017` 自送回收后成功区分数判断与活跃能量奖励已迁到 `workflows/cards/pr-017-nico.ts`。`PL!HS-bp5-001` 公开手牌同名回收、`PL!HS-bp1-002` 从休息室登场、`PL!HS-bp6-017` / `PL!HS-pb1-020` / `PL!-bp6-005` 分组回收、`BP5_003` 支付能量+弃手分支、`HS_PB1_004` 支付能量+弃手+堆墓、reveal helper 仍暂缓；后续重点是 wrapper 化这些复合路径，而不是宣称 steps DSL 或万能 zone move。 |
...
| `F03,F04,F05,F06,F13` | `src/application/effects/look-top.ts` + look-top workflow modules | 看顶/公开/清理/顶牌入休息室原语已外移；无前置费用或费用已由外层完成的看顶入手已迁到 `workflows/shared/look-top-select-to-hand.ts`，弃 1 手牌后看顶入手已迁到 `workflows/shared/discard-look-top-select-to-hand.ts`，`PL!HS-bp5-008-R` 自身待机 + 弃手外层已迁到 `workflows/cards/hs-bp5-008-izumi.ts`。支付能量与分支前置样例仍未统一配置化；这不是 steps DSL，trigger matcher 也未接 runner。 |
