...
|---|---|---|
| `F07,F08,F09` | `src/application/effects/zone-selection.ts` + `runtime/actions.ts` + waiting-room recovery workflows | 当前 `WAITING_ROOM -> HAND` 底层移动已由 `recoverCardsFromWaitingRoomToHandForPlayer` 收束；纯 pending 回收、纯自送回收、支付能量回收、`PL!-bp4-002` 弃手费用回收已迁出 runner，分别落在 `workflows/shared/waiting-room-to-hand.ts`、`self-sacrifice-waiting-room-to-hand.ts`、`pay-energy-waiting-room-to-hand.ts`、`discard-cost-waiting-room-to-hand.ts`。`PL!HS-bp5-001` 公开手牌同名回收、`PL!HS-bp6-017` / `PL!HS-pb1-020` / `PL!-bp6-005` 分组回收、`BP5_003` 支付能量+弃手分支、`HS_PB1_004` 支付能量+弃手+堆墓、`PR_017` 回收后奖励、`HS_SD1_006` 登场活跃能量后回收仍暂缓；后续重点是 wrapper 化这些复合路径，而不是宣称 steps DSL。 |
