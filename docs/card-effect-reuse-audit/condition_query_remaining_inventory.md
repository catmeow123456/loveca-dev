...
- 有效 Heart 高于当前原本 Heart：`memberHasMoreEffectiveHeartsThanOriginal` 已作为纯 domain query 落地，`PL!HS-pb1-029` 与 `PL!HS-PR-028` 共用；同一次判断复用单次 `collectLiveModifiers` 结果。比较基线使用应用最新 `MEMBER_ORIGINAL_HEART_REPLACEMENT` 后的当前原本 Heart，因此印刷0→原本4的 count-changing replacement 本身不成立，只有再叠加普通 Heart modifier 超过原本4才成立。
