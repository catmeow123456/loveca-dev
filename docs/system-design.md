...
- 云端卡组与离线模式并存
- 正式联机房间闭环：创建/加入、云端卡组锁定、双方准备开始、开局猜拳与胜者决定先后手、服务端权威对局、轮询同步、请求式重开、玩家视角只读观战链接、房间号观战入口、离开/短暂恢复与管理员房间观测；房间号观战默认开放双方玩家视角，参赛玩家可在对局中关闭或重新开启自己的视角，关闭时只撤销房间号来源观战会话，不影响显式观战链接；管理员房间监控可打开指定玩家视角只读观战页，且不计入公开观战人数
