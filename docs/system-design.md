...
- `src/server/ai-battle/semantic-context.ts`
...
AI 对战的测试进一步按“领域场景 → 语义行为 → 出站协议 → 运行时集成 → 真实模型/真人抽样”分层。协议版本由生产代码集中导出，场景、observation、语义上下文、模型 envelope 和 fake model 使用彼此分离的公共 builder；完整 snapshot 只冻结真实 provider-neutral 出站边界，行为测试不重复绑定协议版本或整个嵌套 payload。日常重构运行受影响层的 focused tests、类型检查和一场 runtime smoke，完整认证矩阵与高种子 playout 分别在合并前和专用 CI 执行。精确职责与变更矩阵见 [`docs/ai-battle/design-and-implementation.md`](ai-battle/design-and-implementation.md#115-测试架构与变更隔离)。
...
- AI 对战 Phase 0～4 及 Phase 4.5 语义切片：`src/server/services/ai-battle-phase-three-service.ts` 以不可登录 SYSTEM 身份和两个精确认证卡组创建标准 `ONLINE + AI_BATTLE` 对局；`src/server/ai-battle/` 维护 typed contract、allowlist strategy context、语义决策上下文、确定性策略、decision lease、模型协议与调用治理。`src/server/ai-battle/semantic-context.ts` 只从脱敏 observation 与纯净精选历史派生中文局面事实、合法选择和逐动作后果；`src/server/services/online-match-service.ts` 只在战术窗口把不含事实编号的语义正文作为 user 消息，并把压缩规则、完整卡组知识和输出契约作为 system 消息交给固定 provider，内部 fallback playbook 不出站。服务端在锁外校验 typed selection 并从合法选择派生事实审计引用，提交时在锁内重验 revision、窗口、契约与 typed selection，并把脱敏调用事实与命令帧原子记录。连续协议格式失败只降级当前步骤，供应商/传输/预算失败才降级整局。`src/server/ai-battle/model-governance.ts` 用单一序列化函数生成真正送给 provider 的 system/user 消息；开发开关开启时，`debug-trace/v2` 只在当前进程内存保留这些消息与解析结果，管理员路由和 `client/src/components/game/AiBattleDebugPanel.tsx` 将其按请求分区展示，普通参与者、生产环境、聊天和持久记录均不可读取。`client/src/components/pages/AiBattlePage.tsx` 提供登录玩家独立入口，`client/src/store/gameStore.ts` 继续复用标准快照、命令和 `GameBoard`
