...
轮换主题牌桌在同一持久票据/预留状态机上增加独立 `THEME` 队列与活动环境身份。产品与管理界面把活动版本呈现为非计分主题赛季，复用排位的周期、开放窗口、生命周期和管理页结构；管理员可将合法云端卡组或 YAML 复制为不可变服务端预组，并在未结束赛季中继续新增、编辑或移出卡组。每副当前预组自动形成同卡组内战，并与池内既有卡组形成等权组合；编辑以退休旧预组、创建同 `deckKey` 新版本和重建未来组合实现，删除以退休版本和停用组合实现，不回写不可变分配记录或票据锁组。玩家页将服务端公开卡表投影为“卡组选择栏 + 当前卡组卡册”，按成员、LIVE、能量显示唯一卡图、名称和数量，并复用全站只读卡牌详情抽屉；该浏览投影不参与权威分配，后台难度枚举也不投影为玩家标签。冻结组合在双方确认后于事务内完成组合抽取、座位交换、分配记录和票据锁组。玩家本期胜负由 `theme_table_assignments` 关联 `match_records` 的权威终局实时聚合，不新增评分投影、结算流水或排行榜；平局计入完成局数，中断与损坏记录不计入。主题房间禁用原房再战，赛后返回主题入口重新候场，确保每局都有新的分配事实。开局前责任方离场或未到场时，恢复服务终结旧预留，并在活动仍开放时为无过错方创建保留原 `joinedAt` 的新票据。详细边界见[轮换主题牌桌策划与设计](matchmaking-and-ladder/ROTATING_THEME_TABLE_DESIGN.md)。
...
- 本地双人调试模式与对墙打模式：`client/src/components/pages/GameSetupPage.tsx` 按快速匹配、赛季活动和其他对战组织公共牌桌、赛季排位、主题赛季、房间联机、对墙打和双人调试入口，并保留本地模式的分步选组；`client/src/lib/debugPerspective.ts` 与 `client/src/store/gameStore.ts` 在本地双人调试的 `RULES` 模式下按玩家视图权限自动跟随当前操作方，`client/src/components/game/GameBoard.tsx` 同时为桌面和移动端保留显式视角切换与确认式重开入口
- 认证、卡组、卡牌、图片管理 API
...
- 平台状态、公告与玩家入口配置：`src/server/site-status.ts` 定义公开站点状态契约，`src/server/services/site-announcement-service.ts` 组装数据库优先、环境变量兜底的维护状态和公告，并独立读写赛季排位、主题赛季两个玩家入口显示位。`src/server/routes/site-announcements.ts` 提供管理员维护、公告和入口 API，`src/server/routes/app-config.ts` 通过 `/api/config` 暴露公开 `siteStatus` 与 `features.battleEntries`；入口位只控制 `HomePage` 和 `GameSetupPage` 的导航发现，不改变赛季生命周期、队列运行态，也不卸载跨页面候场层。数据库增量由 `drizzle/0027_add_player_battle_entry_visibility.sql` 提供
