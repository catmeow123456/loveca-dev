...
- 每张输入最大 8 MB、单边最大 8192 像素、总像素最大 32 MP。服务端使用 Sharp 解码、应用 EXIF 方向、转为 sRGB、移除元数据并重新编码静态 WebP；默认单步处理超时 30 秒，全局并发默认 2。浏览器和反向代理为壁纸发布单独保留 90 秒请求预算，不改变其他 API 的通用超时。
...
- 客户端在选图和保存前按实际裁切区域检查最低分辨率；服务端仍以结构化错误码作为最终边界，前端将分辨率不足、处理超时和网关失败转换为简短的玩家提示，不直接展示 5xx 技术响应。
