...
| `C03` | P0 | 支付N点能量 | cost: `payEnergy(n, optional?)` | `core_v1` | Stage 1B 已落地横置前 N 张活跃能量；`PL!HS-bp1-004-P` 费用 15「夕雾缀理」已验证起动支付 3 能量与 LIVE 开始可选支付 1 能量，`PL!HS-sd1-006-SD` 费用 15「安养寺姬芽」与 `PL!HS-pb1-004-R` 费用 4「百生吟子」继续验证 LIVE 开始支付 1 能量、登场复合费用中的支付 1 能量。后续需要产生 `ENERGY_PAID` event。 |
...
| `F06` | P0 | 从卡组顶将N张置入休息室 | deck move: top -> waitingRoom | `core_v1` | Stage 1C 已落地检视牌入休息室与卡组顶 N 张直接入休息室原语；`PL!HS-pb1-004-R` 费用 4「百生吟子」验证支付后顶 3 直接入休息室，`PL!HS-PR-019-RM` 费用 2「百生吟子」验证先公开检视顶 3、点击继续处理后再放置入休息室。 |
| `F07` | P0 | 从休息室将卡加入手牌 | zone selection/move | `core_v1` | Stage 1A 第一版已落地到 `src/application/effects/zone-selection.ts`，当前覆盖 `WAITING_ROOM -> HAND` 单选路径；`PL!-pb1-019-N` 费用 2「高坂穗乃果」、`PL!-bp4-003-P` 费用 2「南琴梨」、`PL!-bp4-003-R` 及同型 35 张卡牌已验证非预组起动扩样本。 |
| `F08` | P0 | 从休息室回收LIVE卡 | F07 specialization with selector `type=LIVE` | `core_v1` | Stage 1A 已用 `typeIs(LIVE)` 配置 001/005；`PL!-bp4-003-P` 费用 2「南琴梨」继续复用同一路径，不做单独硬编码函数。`PL!HS-bp1-004-P` 费用 15「夕雾缀理」起动段已验证 `typeIs(LIVE)` + 莲之空 group selector 回收 LIVE；`PL!HS-sd1-006-SD` 费用 15「安养寺姬芽」与 `PL!HS-pb1-004-R` 费用 4「百生吟子」继续验证莲之空 LIVE / Cerise Bouquet LIVE 回收 selector。 |
| `F09` | P0 | 从休息室回收成员卡 | F07 specialization with selector `type=MEMBER` | `core_v1` | Stage 1A 已用 `typeIs(MEMBER)` 与组合 selector 配置 002/003；`PL!-pb1-019-N` 费用 2「高坂穗乃果」与后续同型卡继续复用同一路径。 |
...
| `B05` | P0 | LIVE合计分数增加 | live modifier: total score delta | `core_v1` | Stage 1D 已迁移：当前 009 与 `PL!HS-bp2-022-L+` 分数 2「アオクハルカ」通过 `addLiveModifier` 写入 `SCORE`，自动判定读取 `collectLiveModifiers`。 |
...
| `E02` | P1 | 将能量变为活跃 | energy state step | `core_v2` | Stage 1I 已起步：`src/application/effects/energy.ts` 提供 `setEnergyOrientation` / `setFirstEnergyCardsOrientation`，`PL!SP-bp4-008-P` 费用 13「若菜四季」右侧登场验证最多 2 张待机能量变活跃；Stage 1M 已由 `PL!SP-bp5-003-AR` 费用 17「岚 千砂都」LIVE 开始验证全部能量变活跃；Stage 1N 已由 `PL!N-pb1-008-P+` 费用 17「艾玛·维尔德」验证选择能量分支后自动按能量区顺序处理至多 2 张待机能量；`PL!HS-sd1-006-SD` 费用 15「安养寺姬芽」验证登场条件成立时活跃 1 张能量。能量没有个体差异，不要求玩家逐张选择具体能量卡。 |
...
Stage 1A-1F 已把 `F07/F08/F09`、`C01/C02/C03/C04/E01`、`F03/F04/F05/F06/F13`、`F01`、`B03/B05/B07/B08/T05`、`S01/S02/S05` 的当前验证集主路径落到模块底座。Stage 1I 已把 `E03` 的能量卡组放置与 `E02` 的能量方向变更起步到底座，并用 `requiredSourceSlots` 打开来源槽位条件过滤。Stage 1J 已用 `PL!SP-bp4-008` 费用 13「若菜四季」左侧登场与 `PL!HS-bp1-006` 费用 11「藤岛 慈」登场验证 `F02` 抽 2 弃 1 组合步骤，并用 `PL!-pb1-019` 费用 2「高坂穗乃果」和 `PL!-bp4-003` 费用 2「南琴梨」验证自送休息室后的成员/LIVE 回收扩样本。Stage 1K 已补完 `PL!SP-bp4-008` 费用 13「若菜四季」LIVE 开始可选站位变换，继续复用 `member-state.ts`。Stage 1L 已用 `LL-bp2-001` 费用 20「渡边 曜&鬼冢夏美&大泽瑠璃乃」、`PL!N-pb1-008` 费用 17「艾玛·维尔德」和 `PL!SP-bp5-003` 费用 17「岚 千砂都」打开 `X11` 登场费用修正底座。Stage 1M 已用 `PL!SP-bp5-003` 费用 17「岚 千砂都」与 `PL!S-bp2-006` 费用 11「津岛善子」验证批量活跃与 `S07` 卡效登场边界。Stage 1N 已用 `PL!N-pb1-008` 费用 17「艾玛·维尔德」验证 `X03` 二选一目标类型，并复用 `S02/E02` 方向 helper。Stage 1O 已用 `PL!HS-bp2-012-N` 费用 5「乙宗 梢」与 `PL!HS-bp6-017-N` 费用 11「日野下花帆」打开 AUTO / `S08` 离场触发 proving path；Stage 1P 已用 `PL!HS-pb1-009` 费用 15「日野下花帆」打开 `ON_ENTER_STAGE` 监听型 AUTO、`T07` 实例级每回合限制、`B01` BLADE modifier、成员有效 BLADE helper、`F02` 阈值抽弃与 confirm-only 无输入确认壳；`PL!HS-bp1-006` 费用 11「藤岛 慈」已补齐 LIVE 开始弃手后按其他成员条件选择 Heart 的 B03 扩样本；`PL!HS-bp1-004` 费用 15「夕雾缀理」已补齐 C03/F08 起动回收与 C03/B01 LIVE 开始 BLADE 扩样本；本批 `绿莲-6弹ver.yaml` 已补齐 `PL!HS-bp5-019` 分数 6「花结」、`PL!HS-bp2-022` 分数 2「アオクハルカ」、`PL!HS-sd1-006` 费用 15「安养寺姬芽」、`PL!HS-bp5-008` 费用 4「桂城泉」、`PL!HS-pb1-004` 费用 4「百生吟子」、`PL!HS-PR-019` 费用 2「百生吟子」，覆盖 LIVE 卡来源 modifier、别名归一化、C05/C06 复合费用、公开检视顶 3 后确认入休息室与固定绿色 Heart。卡效登记已支持 `baseCardCodes`，同基础编号不同罕度由 `tests/unit/card-effect-rarity-sync.test.ts` 防漏同步；`existing_module_map.md` 也已重写为按基础编号记录的完成状态登记册。
