...
| `createEnterWaitingRoomEvent`                                                                                                                   | 创建卡牌进入休息室事件，支持同批多张进入休息室的 `cardInstanceIds`。             | `cardInstanceId` 保留首张兼容旧读取；弃手 helper 会记录手牌到休息室事件，workflow 在正确结算点显式把本次事件传给 runner 消费。                                                                                              |
| `enqueueEnterWaitingRoomTriggersFromDiscardResult`                                                                                              | 将弃手 helper 返回的本次进入休息室事件显式交给 runner 入队。                     | 只消费 `discardResult.enterWaitingRoomEvent`；不从 `eventLog` 查 latest/all，不 resolve pending，不改变当前 workflow 后续 step。                                                                                            |
...
- helper 不自动调用 `enqueueTriggeredCardEffects`，需要触发“手牌进入休息室”自动能力的 workflow 必须只消费 helper 返回的本次新事件；现有弃手 workflow 统一复用 `enqueueEnterWaitingRoomTriggersFromDiscardResult` 做这层胶水。
