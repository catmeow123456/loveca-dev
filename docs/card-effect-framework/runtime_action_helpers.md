...
| `stackEnergyFromEnergyZoneBelowMember`                                                                                                          | 将指定玩家 energyZone 的 N 张能量自动放到目标主成员下方。                                     | 位于 `src/application/effects/energy-below.ts`；统一采用 WAITING-first 自动选择：先按 `energyZone.cardIds` 原顺序取 WAITING，数量不足再按原顺序补 ACTIVE；不打开玩家选择 UI，也不是卡牌专属规则。能量总数不足或目标槽无主成员时返回 `null` 且不部分移动；`count=0` 保持 no-op。返回 `stackedEnergyCardIds` 供 PAY_COST / RESOLVE payload 使用；这不代表完成通用能量费用或能量选择框架。 |
