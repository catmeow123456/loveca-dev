...
| `moveWaitingRoomCardsToDeckBottomForPlayer`    | 将 caller 已有序确定的休息室卡按传入顺序放到主卡组底。             | 固定 `WAITING_ROOM -> MAIN_DECK_BOTTOM`；候选集合必传；不洗切、不扫描 selector、不写 action、不处理 activeEffect。                                                                                                                                                                                                                                                                                       |
...
- 目前真实样本是 `PL!N-bp4-009` 抽 2 后强制选择 1 张当前手牌回顶；多张排序、可选回顶、公开手牌或其他来源区需要另审。
