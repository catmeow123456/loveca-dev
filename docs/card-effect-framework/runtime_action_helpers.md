...
| `moveWaitingRoomCardsToDeckBottomForPlayer`    | 将 caller 已有序确定的休息室卡按传入顺序放到主卡组底。             | 固定 `WAITING_ROOM -> MAIN_DECK_BOTTOM`；候选集合必传；不洗切、不扫描 selector、不写 action、不处理 activeEffect。真实 0–1 LIVE 样本为 `PL!S-bp2-008` 费用 17「小原鞠莉」。                                                                                                                                                                                                                                                                        |
| `moveWaitingRoomCardsToDeckTopForPlayer`       | 将 caller 已有序确定的休息室卡按传入顺序放到主卡组顶。             | 固定 `WAITING_ROOM -> MAIN_DECK_TOP`；候选集合必传；不洗切、不扫描 selector、不写 action、不处理 activeEffect。                                                                                                                                                                                                                                                                                          |
| `moveWaitingRoomCardToDeckPositionForPlayer`   | 将 caller 已确定的 1 张休息室卡插入主卡组第 N 张位置。             | 固定 `WAITING_ROOM -> MAIN_DECK`；目标位置从卡组顶 1-based 计数，剩余卡组不足时插入底部；不扫描 selector、不写 action。                                                                                                                                                                                                                                                                                  |
...
| `stackMemberCardBelowSpecialMember`            | 将指定玩家手牌或休息室的 1 张成员卡放到舞台 special member 下方。  | 固定 `HAND / WAITING_ROOM -> memberBelow`；显式校验 host 在舞台且为 special member、移动卡是该玩家成员卡、来源区正确；不走普通登场回退，不 enqueue trigger。                                                                                                                                                                                                                                             |
| `playMemberBelowCardToEmptySlot`               | 将指定 special member 下方的 1 张成员卡登场到空成员区。            | 固定 `memberBelow -> empty MEMBER_SLOT`；校验来源卡确在指定 host/sourceSlot 的 `memberBelow`、目标槽为空且移动卡为成员；写入 FACE_UP / ACTIVE 与 `ON_ENTER_STAGE` 事件，但不自动入队 trigger。                                                                                                                                                                                                           |
| `playMemberFromZoneToStageSlotWithReplacement` | 将 HAND / WAITING_ROOM 的 1 张成员移动到指定成员区，并完成可选的单成员替换。 | 只校验来源区、owner、MEMBER 与 `canMemberBeRelayedAway` 的最终防线；空区域直接登场，覆盖区域时先读取原成员有效费用，将 energyBelow 返回能量卡组，将原成员与 memberBelow 放置入休息室，再写标准 LeaveStage / EnterWaitingRoom / EnterStage 事件与单 replacement 快照。caller 决定费用、团体、阈值、区域合法性、窗口、action、trigger enqueue 与 pending continuation；不扩成 replacement DSL。当前真实调用者为 `PL!SP-sd1-002` 与迁移后的 `PL!N-bp1-002`。 |
...
| `collectContinuousLiveProhibitionSources` / `isPlayerContinuouslyLiveProhibited` / `isPlayerLiveProhibited` / `liveProhibitedPlayerLiveZoneToWaitingRoom` | 从当前场面实时收集常时禁止来源，并把临时或常时任一命中统一交给 Live Set 抽牌后的既有处理。 | `PL!SP-bp1-001` 是首个 continuous 样本：只读取控制者三个主舞台顶层合法 MEMBER，单独一张合法 001 时禁止；其他己方顶层成员使其即时失效，对方成员与 memberBelow 不计入，两张 001 互为其他成员。查询不写状态、不创建 action、不进入 `liveModifiers`。`SET_LIVE_CARD` 仍不拒绝盖牌，按规则抽牌后再把受限玩家 liveZone 全部放入休息室；不要扩展成任意 condition callback、期限 DSL 或 phase prohibition framework。 |
...
| `partitionInspectedCardsToHandDeckTopWaitingRoomAndEnqueueTriggers`                                                                             | 将 caller 已分配的 inspected cards 原子移动到手牌、卡组顶与休息室。                           | 校验三个目的地无重复且恰好全量覆盖传入 inspected cards，并确认每张仍在 inspectionZone；一次性更新三个目的地并统一清理 inspection。只为 waiting-room 子集发同批 `MAIN_DECK -> WAITING_ROOM` 事件；不限定各目的地数量、不表达单卡分配规则、不创建 activeEffect 或推进 pending。                                                                                   |
...
## Special Member Stack Helper Parameters
...
| ------------- | ---------------------------------- |
| `game`        | 当前 `GameState`。                 |
| `playerId`    | 来源区与 special member 所属玩家。 |
| `sourceZone`  | `HAND` 或 `WAITING_ROOM`。         |
| `movedCardId` | 要移动到下方的成员卡实例。         |
| `hostCardId`  | 目标槽位上的 special member 实例。 |
...
- 只覆盖 1 张成员卡从手牌或休息室放到 special member 下方。
- 显式校验 player 存在、host 在该玩家舞台目标槽、host card 是 `isSpecialMemberCard`、移动卡属于该玩家且是成员卡、移动卡当前在声明来源区、且不能重复移动已在 `memberBelow` 中的卡。
- 当前 special member host 白名单由 `isSpecialMemberCard` 管理；已登记的 PB2 host 包括 `PL!SP-pb2-005` 与 `PL!SP-pb2-006`。新增 host 时只更新白名单与对应卡效测试，不放宽 helper 的 host 校验语义。
...
| ------------ | ------------------------------------- |
| `game`       | 当前 `GameState`。                    |
| `playerId`   | special member 所属玩家。             |
| `hostCardId` | 来源槽位上的 special member 实例。    |
...
- 只覆盖 1 张成员卡从指定 special member 下方登场到自己的空成员区。
