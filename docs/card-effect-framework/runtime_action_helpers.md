...
| `stackEnergyFromEnergyZoneBelowMember`                                                                                                          | 将指定玩家 energyZone 前 N 张能量自动放到目标主成员下方。                                     | 位于 `src/application/effects/energy-below.ts`；不打开选择 UI，不区分 ACTIVE / WAITING；能量不足或目标槽无主成员时返回 `null` 且不移动；内部复用 `removeCardFromStatefulZone` 与 `addEnergyBelowMember`，返回 `stackedEnergyCardIds` 供 PAY_COST / RESOLVE payload 使用。                                                                                                                                              |
