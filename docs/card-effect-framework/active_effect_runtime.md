...
| `createOptionalDiscardHandToWaitingRoomActiveEffect`                                             | `src/application/card-effects/runtime/active-effect.ts`                                 | 构造“可选精确弃固定数量手牌到休息室”的 activeEffect shell，统一 step text、候选可见性、selection/skip label、精确选择数与 `effectCosts` / `handToWaitingRoomCost` metadata；`discardCount` 缺省为1。 | 只返回 `ActiveEffectState`；不移除 pending，不写 action，不执行弃牌，不支付费用，不处理额外费用、按类分组选择、hand-adjust、skip 或 pending continuation。 |
...
`createOptionalDiscardHandToWaitingRoomActiveEffect` is only the reusable selection-window shell for optional exact fixed-count hand discard costs. `discardCount` defaults to 1; values above 1 build one exact multi-select window and exact-count cost metadata. The caller still decides when to remove the pending ability, what action payload starts the window, how skip resolves, and how the selected cards are discarded later. Do not use it for windows with extra energy/source costs, category-grouped selection, discard-to-N hand adjustment, or other effects whose metadata would require a mini configuration interpreter.
