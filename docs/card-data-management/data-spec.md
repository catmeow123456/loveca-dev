...
```json
[{ "effect": "HEART", "heartColor": "PURPLE" }, { "effect": "DRAW" }, { "effect": "SCORE" }]
```
...
- 后端 `POST /api/cards` 创建的新卡默认是 `DRAFT`，路由 schema 支持请求体显式传入 `status`。
- 当前 Admin UI 和 `cardService.createCard()` 不暴露 `status` 输入，因此管理界面新建卡会走默认 `DRAFT`。
- 当前维护的 `src/scripts/sync-cards-llocg.ts` 同步脚本会把新插入卡牌固定写为 `PUBLISHED`；已有卡牌如字段有差异，会先进入 TTY 人工审核，审核通过后更新并同样写为 `PUBLISHED`。
- 因此，同步脚本不属于“草稿导入后再逐张上线”的流程；若需要草稿导入，应另行调整脚本或使用管理接口导入。
