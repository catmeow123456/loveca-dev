...
| 格式        | 含义                               |
| ----------- | ---------------------------------- |
| `sd` + 数字 | 起始卡组，如 `sd1`、`sd2`、`sd3`   |
| `bp` + 数字 | 补充包，如 `bp1`、`bp8`、`bp10`    |
| `cl` + 数字 | Collection/联动商品，如 `cl1`      |
| `pb` + 数字 | 购入特典包，如 `pb1`、`pb2`、`pb3` |
| `PR`        | PR 卡                              |
| `E`         | 能量卡（仅 `LL-E-*`）              |

数字编号商品不使用逐个白名单；只要符合上述格式，未来新商品无需为卡号校验额外修改代码。
...
当前 `HeartIconSchema` 允许 `GRAY` 与 `RAINBOW` 出现在 `HeartIcon[]` 中。`GRAY` 表示实际无色 Heart 资源，不可作为其他指定颜色；`RAINBOW` 表示可代替任意颜色的 All Heart。
...
| --------- | ---------------------------------- |
| `GRAY`    | 无色/泛用总数需求                         |
| `RAINBOW` | 旧有 `heart0` 结构化表达，在需求中同样归一为泛用总数 |

需求语义与提供的 Heart 资源语义不同：需求中的 `GRAY` / `RAINBOW` 条目都表示无指定颜色的总数需求；提供的 `RAINBOW` Heart 仍然可补指定颜色，提供的 `GRAY` Heart 不可以。
...
| ------- | ------------------------------ | ------------------ |
| `HEART` | 必填，颜色枚举值（含 GRAY / RAINBOW） | 声援时公开的 Heart |
| `DRAW`  | 不填                           | 声援时抽一张卡     |
| `SCORE` | 不填                           | Live 成功时分数 +1 |

- 成员卡: 通常只有 `HEART` 效果
- Live 卡: 通常只有 `DRAW` 或 `SCORE` 效果（来自 llocg_db 的 `special_heart`）
- Loveca Excel / CloudBase 中的 `double` 在同步时展开为两条 `{ effect: 'HEART', heartColor: 'GRAY' }`，管理端也以这两条规范化记录展示和保存。
...
- 数据库不再保留重复的 `group_name` 列；运行时不再派生 `groupName` 展示值，前端展示直接读取 `group_names` / `groupNames` 并按需拼接。
