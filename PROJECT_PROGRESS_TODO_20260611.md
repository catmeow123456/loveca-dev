...
- 优先梳理 Live 判定/分数自动化流水线：当前项目已有 `src/domain/rules/live-resolver.ts` 这类自动判定雏形，但实际桌面仍以“信任玩家”手动确认为主；后续要做加棒、加心、减要求心数、+1 分等卡效前，建议先把判定和分数自动化接进主流程。
- Live 判定自动化方向：
  - 翻判定牌自动化：根据成员光棒数自动翻对应数量应援牌。
  - Live 是否成功自动化：用 `LiveResolver.judgeMultipleLives` 或类似逻辑，根据 Heart 池自动判断成功/失败。
  - 分数自动化：成功 Live 分数 + 应援音符加分自动计算。
  - 人工修正保留为调试/例外按钮：例如“调整分数”“强制成功/失败”，但默认流程应自动处理。
- 卡效接入点预留：加棒/加心常时效果、减要求心数的 `LIVE开始时` 效果、`LIVE开始时`/成功时 `+1分` 效果，都应进入同一套自动判定/分数计算流水线，避免 UI 手填结果导致卡效无法真正自动化。
- 统一早期效果选择 UI：检查 `PL!-sd1-004-SD` 海未选择 LIVE 加入手牌、多个 `LIVE开始时` 效果选择发动顺序等仍只显示名字的旧选择入口，尽量都改成卡图网格 + hover 详情；文字标签只作为辅助信息。
- 图片加载策略收口：当前为方便 5176 本地卡效测试，暂时保留 `assets/images` 本地 fallback，同时兼容作者的 MinIO 图片代理；正式上线/完整测试环境稳定后，评估删除本地 fallback，统一走后端/MinIO 图片服务。
