...
- Phase 3 已完成。`ai-battle.system-participant-identity/v1` 建立不可登录 SYSTEM 身份，并按局绑定精确 YAML 内容哈希、Phase 0 认证版本、decision contract / command adapter、压缩规则、playbook、strategy context、无 LLM 可解释策略及赛前/生命周期政策版本；Phase 4 曾按停机升级原则将身份提升为 v2 并追加模型边界版本，Phase 4.5 第一批切片再将当前身份提升为 v3，追加 model strategy context 与 semantic decision context 版本绑定。卡牌规则与卡牌数据变更由版本发布、Phase 0 认证和 focused tests 共同管理；不另建重复的 runtime 规则语义哈希。`ONLINE` SYSTEM 只有在 `AI_BATTLE` 且存在单一完整认证 binding 时才能建局和进入全局机器调度，普通入口、未绑定或篡改 binding 均被拒绝，`SOLITAIRE` 不进入战术调度。正式主策略只保留 authority progress watchdog，保守策略的 3 回合/256 决策/5 分钟阈值只在真实降级后计数。管理员受控入口继续使用标准 `ONLINE` runtime、投影、聊天、历史与共享猜拳规则；八个认证单元均验证自然规则终局、正式策略记录存在、所有策略提交零权威拒绝且未触发机器故障/活性认输。机器可读完成基线见 `phase-three-baseline.ts`。普通玩家入口、LLM 调用和公共牌桌补位不在本阶段范围内。
...
- Phase 4.5“语义决策上下文与事实自检”已进入实施，仍是 Phase 5 之前的最高优先级。第一批后端切片已经新增 `semantic-decision-context/v1`，只从 allowlist observation 与纯净精选历史生成中文局面事实、稳定 `factId`、合法选择和逐动作后果；`model-request-envelope/v2` 实际只发送规则/playbook 与 `model-strategy-context/v1` 的语义正文，不再直接发送原始 observation/history。`model-decision-output/v2` 要求结构化选择、事实引用、简短取舍和下一步计划，引用会在提交前检查存在性与所选方案必要事实，并复用一次有限修复；当前脱敏审计/记录同步升级为 `strategy-decision-audit/v3` 与 `strategy-decision-record/v4`。`selected-history/v3` 的已接受决定摘要改由脱敏 observation + 结构化选择/动作派生，模型文本不再作为事实回灌；换手回归已固定验证来源能力、被替换对象、动作后槽位和剩余能量。管理员上下文检查器也已落地：开发开关开启时，仅管理员可在自己进入的 AI 测试对局中逐次查看与真实调用同源的 provider-neutral system/user 消息、结构化语义分区、输出契约与严格解析结果；上下文只留在当前进程内存，不写入聊天、录像或数据库，也不保留原始无效 provider 响应。当前仍未完成更广卡效/可选费用/目标选择语义回归、真实 provider 新协议评测与真人抽样复盘，因此 Phase 4.5 仍为 `IN_PROGRESS`。
...
3. 公共牌桌 Beta 的进行中房间跨进程恢复、开局到场超时后的无过错方自动回队、完整指标聚合与运行后台仍需收束；赛季排位首版、V1 生产开放、当前赛季 V3 切换及基础运营概览已经完成，当前缺口是外部告警渠道、跨日运营趋势以及 V4 新赛季上线前的回放参数验证；生产报告仍不能细分旧样本中 34 条非终局记录的具体状态。AI 对战已完成 Phase 0～4，Phase 4.5 的语义决策上下文、基础事实引用自检、纯净精选历史和管理员实际请求上下文检查器已落下第一批切片，但更广语义回归、真实 provider 新协议评测和真人抽样质量回归仍未完成；其完成前不推进 Phase 5。Phase 5 的公共牌桌补位、presence、多实例互斥、进行中恢复、跨实例模型容量与运营监控也尚未实现。
...
1. 继续收束 AI 对战 Phase 4.5：在已落地的权威局面/行动后果语言化、基础事实引用校验、模型文本/事实历史隔离和管理员实际请求上下文检查器之上，补齐更广卡效语义回归、真实 provider 新协议评测与真人抽样复盘；策略质量以事实一致性、取舍完整性和计划连贯性验证，不建设通用收益值或后悔值裁决器。
