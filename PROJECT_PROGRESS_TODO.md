...
- 赛季卡牌使用率与卡组分类环境已形成开发基线：排位注册事务会将双方锁定主卡组规范化为长期观察事实，不同罕度按基础编号合并，并保留可重建分类所需的稳定构筑指纹。玩家页按赛季提供卡牌使用占比、胜者构成和高排名玩家三个各自独立的 Top 30 TAB；卡组/卡牌各自保存展示内容和基础计权、仅共享 N，卡牌统计不依赖分类发布，而“胜者构成”统一表达胜方使用了什么，不是胜率。分类发布快照冻结分类名称与规则，`UNKNOWN / AMBIGUOUS` 保持独立；管理员玩家积分查询的排名上下文表为目标及上下各最多 3 名逐行展示胜场、负场和当前 `ACTIVE` 发布口径下的最常用卡组分类，并明确并列与分类覆盖。管理员对局处理页可按需展开双方长期主卡组，并明确标识早期缺失观察的历史对局。`0020_add_ranked_deck_observations.sql` 提供长期事实，最终单一 `0032_add_deck_classifier.sql` 提供分类、发布、assignment 与两套独立展示设置；能量卡组与逐张实例不属于长期观察。代码、聚焦测试、类型检查、前端构建和本地现有赛季只读 API 已完成验收；生产仍需按对应 runbook 执行首次 0032、种子导入与分类发布。
