...
- 轮换主题牌桌第一版开发闭环已落地：独立 `THEME` 队列、活动/预组/组合/分配版本、确认后事务内组合与座位分配、服务端预组锁定、玩家节目单与跨页面候场均已接入现有公共牌桌和正式房间；网站与运营后台统一称为“娱乐模式”，内部技术命名保持不变。玩家页按活动版本从权威终局展示本人完成局数、胜负平与胜率，不建立积分、段位或排行榜；当前卡组版本的两两对阵改为节点关系图，预组为节点、对局为连线、连线中显示双方胜局比，座位交换后仍按真实卡组聚合，退休版本不混入当前图，自组对局不绘制为自环。预组池以卡组选择栏和响应式卡图卡册呈现，按成员、LIVE、能量显示名称与数量，并复用可访问的只读卡牌详情抽屉，后台难度枚举不再作为玩家标签展示。管理员后台复用排位的“概览 / 活动”框架，主流程收敛为填写娱乐模式活动、维护卡组池和开始活动；未结束活动均可从合法云端卡组加入或直接导入 YAML、移除卡组，并复用现有卡组编辑器调整构筑。新增卡组自动生成同卡组内战及与池内其他卡组的等权组合；编辑会退休旧预组版本并生成仅供后续分配的新版本，移除会停用相关组合，已经分配和锁组的对局保持原版本。逐组停用只保留在折叠的异常处理。开局前责任方离场或未到场时，无过错方会保留原 FIFO 时间取得可审计的新主题票据。后台已提供基础漏斗计数和目标/实际预组曝光。当前仍是开发/内测基线；真实预组试打矩阵、跨进程房间恢复、完整组合指标/告警和双浏览器验收未完成前不得开放生产活动。
...
- 赛季卡牌使用率与卡组分类环境已形成开发基线：排位注册事务会将双方锁定主卡组规范化为长期观察事实，不同罕度按基础编号合并，并保留可重建分类所需的稳定构筑指纹。玩家页按赛季提供卡牌使用占比、胜者构成和高排名玩家三个各自独立的 Top 30 TAB；卡组/卡牌各自保存展示内容和基础计权、仅共享 N，卡牌统计不依赖分类发布，而“胜者构成”统一表达胜方使用了什么，不是胜率。分类发布快照冻结分类名称与规则，`UNKNOWN / AMBIGUOUS` 保持独立；管理员玩家积分查询的排名上下文表为目标及上下各最多 3 名逐行展示胜场、负场和当前 `ACTIVE` 发布口径下的最常用卡组分类，并明确并列与分类覆盖。管理员对局处理页可按需展开双方长期主卡组，并明确标识早期缺失观察的历史对局。`0020_add_ranked_deck_observations.sql` 提供长期事实，最终单一 `0032_add_deck_classifier.sql` 提供分类、发布、assignment 与两套独立展示设置；能量卡组与逐张实例不属于长期观察。生产仍需按对应 runbook 执行首次 0032、种子导入与分类发布。
...
- 前端外层 P1/P2 稳定化已进入开发基线：版本发现统一收口到 prompt 型 `UpdateCoordinator`，应用先渲染，Service Worker 与 `version.json` 只标记更新可用；进行中本地/远程对局不展示更新入口，只有用户在安全页面确认后才允许单次刷新，不再清空卡图、表情等运行时缓存。云端卡组继续由唯一 `deckStore` 持有，并按登录用户隔离 30 秒会话 freshness、合并同用户并发请求，区分冷加载与保留旧数据的后台刷新；首次卡组读取与 PT `ensureLoaded()` 并行，挂载、手动刷新和写后确认已有明确边界。本地 production 双 build PWA 验证已确认新版本发现不会自动导航、确认后只刷新一次且运行时缓存保留；正式发布前仍需用真实账号完成大厅/准备页/卡组页往返请求计数，以及本地对局、服务端对墙打、正式联机、普通网页、PWA、无痕与移动端矩阵验收。
...
- 赛季管理员角色已进入开发基线但尚不可上线：共享 `user / season_admin / admin` 与显式 permission 矩阵、特权请求数据库当前角色复核、独立用户分页/角色变更接口、refresh token 撤销、最后一个平台管理员并发保护、赛季管理审计表骨架和停机迁移已落地；排位、主题赛季和入口显示已声明窄权限，前端按权限投影赛季运营中心及平台管理员用户管理页。角色可在账号列表中直接选择并保存，不要求原因或持久审计。赛季写操作的持久审计、破坏性操作原因输入、完整负向矩阵与 E2E 尚未完成前，不得向真实运营账号分配 `season_admin`。
...
3. 公共牌桌 Beta 的进行中房间跨进程恢复、开局到场超时后的无过错方自动回队、完整指标聚合与运行后台仍需收束；赛季排位首版、V1 生产开放、当前赛季 V3 切换、基础运营概览、玩家胜负/最常用分类查询、卡牌环境三 TAB、卡组/卡牌独立展示设置及单方操作超时开发闭环已经完成，当前排位缺口是上述增量的生产迁移与发布验收、操作超时发布后的生产日志验证、外部告警渠道、跨日运营趋势及 V4 新赛季上线前的回放参数验证；生产报告仍不能细分旧样本中 34 条非终局记录的具体状态。整站维护还需完成生产反向代理接线和真实 PWA/停机恢复矩阵验证。AI 对战基础设施尚未实现。
4. 轮换主题牌桌还缺首期冻结预组与真实实测组合、主题身份历史展示、完整等待/确认/组合胜率与异常率指标、曝光偏差告警、跨进程恢复和双浏览器验收；开局后的断线裁定应复用排位现有的 1 分钟重连期限与在线代际保护，单方超时由超时方判负并写入主题赛季胜负统计，但不引入积分、段位或排行榜；当前实现不得直接作为生产活动开放。
