...
- 赛季排位首版闭环已落地，名为 V1 的首个赛季已在生产开放，当前使用 `GLICKO1_PER_MATCH_V2`。正式 V1/V2 保持 `ratingScale=400` 以确定性重建旧流水，新赛季默认 V3 保持初始 `1500 / RD 300` 和定级参数、只将 `ratingScale` 调为 800；单一 `0010_add_ranked_system.sql` 数据迁移、赛季生命周期/开放窗口、独立排位票据与稳定 FIFO、幂等且可补偿的 `RANKED` 开局绑定、带在线代际保护的断线裁定、权威终局自动结算、追加式 `VOID / REPLACEMENT`、签名更正预览、软重置种子和确定性重建均已实现。排位重连期限为 1 分钟：双方都超时且最后在线时间相差不超过 5 秒时为平台无结果，超过 5 秒时由较早离线方判负；该政策作为不可缺省的赛季公告显示在赛季页、候场状态和局内房间菜单。软重置默认使用新赛季初始积分/RD，草稿期可由管理员选择并参数化向中心值保留公式；策略和参数随竞技环境冻结，种子积分在玩家首场结算前不展示。进入 `FINALIZING` 前已经形成的配对可继续开局，未开局预留消化完之前不能封存；收口任务先排空可靠结算，不能只改数据库状态终止仍在运行的对局。玩家侧已有精简赛季页、跨页面候场/确认、首场积分与变化回显、可配置参榜场次门槛、个人战绩、最近对局和排行榜；管理员侧已有“赛季 / 对局处理”页面，可在草稿或进行中赛季的前端表单配置 1–100 场参榜门槛，并修改名称和开放时段；对局处理默认分页读取全部赛季，支持赛季筛选、用户搜索并直接展示双方胜负、胜方与结果类型。参榜门槛只筛选排行榜，不改变竞技环境或重算积分；算法、竞技环境与赛季时间等计分事实保持冻结。同一活动赛季 V2→V3 使用停机、默认 dry-run 的追加式迁移：不改写或删除旧评分事件与步骤，为每局最新指令追加同结果 V3 `REPLACEMENT / VOID`，再统一重建投影；迁移前必须暂停候场并清空 `PENDING` 对局及活动票据、预留和占用，详见 `RANKED_V3_ACTIVE_SEASON_MIGRATION.md`。V3 代码和迁移工具已实现，生产 V2→V3 迁移尚未执行。
...
- PT 限制已改为数据库版本表：初始数据保留北京时间 2026-04-03 00:00:00 生效表，并排期 2026-08-08 00:00:00 的新表。管理员可在“卡组规则管理”中复制、编辑任意状态的表、预览差异、立即/定时发布、取消排期、废弃/替换与删除已退役表；任何成功操作后都保留且仅保留一张 ACTIVE 表。卡组保存、候场锁定和对局快照保留当时的 PT 版本及点数事实，服务端始终是准入权威。
...
3. 公共牌桌 Beta 的进行中房间跨进程恢复、开局到场超时后的无过错方自动回队、完整指标聚合与运行后台仍需收束；赛季排位首版和 V1 生产开放已经完成，当前缺口是活动赛季 V2→V3 停机迁移演练与执行、告警渠道和运营指标看板，生产报告仍不能细分旧样本中 34 条非终局记录的具体状态。AI 对战基础设施尚未实现。
...
1. 赛季排位下一步是按 `RANKED_V3_ACTIVE_SEASON_MIGRATION.md` 对当前 V1 赛季完成 V2→V3 dry-run、报告复核和停机演练，补齐告警与运营指标；经独立生产授权后再执行迁移。公共牌桌继续收束配对确认超时、房间引导失败恢复、开局失联恢复、维护状态矩阵和聚合指标读取。
