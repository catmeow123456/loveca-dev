...
- 轮换主题牌桌第一版开发闭环已落地：独立 `THEME` 队列、主题赛季/预组/组合/分配版本、确认后事务内组合与座位分配、服务端预组锁定、玩家节目单与跨页面候场均已接入现有公共牌桌和正式房间；玩家页按主题版本从权威终局展示本人完成局数、胜负平与胜率，不建立主题积分、段位或排行榜；预组池以卡组选择栏和响应式卡图卡册呈现，按成员、LIVE、能量显示名称与数量，并复用可访问的只读卡牌详情抽屉。管理员后台复用排位的“概览 / 赛季”框架，主流程收敛为填写主题赛季、选择卡组池和开始赛季，卡组加入时自动生成等权组合，逐组停用只保留在折叠的异常处理；发布后事实不可原地修改。开局前责任方离场或未到场时，无过错方会保留原 FIFO 时间取得可审计的新主题票据。后台已提供基础漏斗计数和目标/实际预组曝光。当前仍是开发/内测基线；真实预组试打矩阵、跨进程房间恢复、完整组合指标/告警和双浏览器验收未完成前不得开放生产活动。
...
- 管理员能力已收敛到统一“运营管理中心”，按内容与平台、卡牌与规则、对局与赛季组织并复用既有模块页面；主页不再平铺六个管理入口。卡牌效果 AI 提取已改为管理员专用服务端链路：PostgreSQL 保存 revision 配置和 AES-256-GCM Key 密文，部署环境控制主密钥、上游白名单与资源硬上限，候选测试不保存，浏览器只提交卡号并接收待确认文本，旧 DashScope 开发代理已移除。
...
4. 轮换主题牌桌还缺首期冻结预组与真实实测组合、主题身份历史展示、完整等待/确认/组合胜率与异常率指标、曝光偏差告警、跨进程恢复和双浏览器验收；当前实现不得直接作为生产活动开放。
