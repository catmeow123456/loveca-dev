...
## 2026-07-23：UI 实现审计问题修复

- 联机房间、调试页与管理员监控改用非重入串行轮询；管理员列表、观战房间查询和云端卡组列表增加 latest-request-wins，输入变化会使旧观战查询失效。
- 修复卡牌管理原地排序、GameBoard / 分数确认 / 卡牌表单 / 响应式侧栏等无 key 草稿与首帧旧状态，并将 render 期间的时间、mutable ref 和外部读取移入可追踪 state / effect；提交前复核另修正桌面端侧栏初值，保持首次进入时默认展开。
- 登录冷却、换牌长按和复制提示补齐 timer 清理；Confirm、分数确认、对局说明弹层统一焦点圈定、焦点恢复和栈式 body scroll lock；根部启用 reduced-motion，并补 CSS 媒体查询。
- 主要页面改为懒加载，主 JS 从约 4,496.91 kB / gzip 971.07 kB 降至约 3,424.42 kB / gzip 773.08 kB；仍保留大 chunk 警告，后续继续拆分全局 store 拉入的 battle runtime。
- 验证：focused Vitest 10 files / 69 tests、全量 Vitest 547 files / 5408 tests 通过，3 个 performance files / tests 按默认配置跳过；shared / server / client TypeScript、客户端 ESLint、生产构建和 `git diff --check` 通过。新增观战 stale response、响应式侧栏与 reduced-motion 浏览器回归；完整四尺寸 Playwright 72 tests 中 61 通过、11 个按视口跳过、0 失败。

## 2026-07-23：移动端战场居中与能量牌架优化（未提交）

- 移动端玩家桌改为等宽左右辅助轨，中央 LEFT / CENTER / RIGHT 成员区与 LIVE 区严格对齐屏幕中线；左侧成功 LIVE 与能量区缩为辅助信息尺度，成员/LIVE 槽仅在窄屏小幅收窄，桌面布局保持原尺寸。
- 移动端能量区改为紧凑四列牌架，单卡放大到 16×24px，待机横置允许相邻卡轻微叠边；桌面与移动端“全活 / 全待”统一为低饱和分段工具条和浏览器实际 10px 字号，移动端只在可操作的己方视角显示，观战与对手视角不保留无意义占位。
- PC 端撤销按钮从成功 LIVE 区移入右下角横向阶段工具条，避免把全局操作误解为成功区操作；不可撤销时仍可点击并展示规则层禁用原因，可请求时立即提示正在发送，服务端成功或失败日志会回显为等待回应或具体错误，并保留 `aria-disabled` 与按压反馈。反馈气泡向上展开，不会越出视口。
- 移除手牌区右侧固定偏移的抽卡/置顶箭头：桌面端“抽 1 张”附着在主卡组下方，“放回牌库顶”在其正下方等宽对齐；主卡组、休息室、能量卡组标题统一高度与基线。移动端把“抽 1 / 放回卡组顶”收进手牌信息行并左右排列；放回操作始终可见，但只有明确选中一张手牌时才启用，命令成功后才清除选择。
- 抽牌与放回牌库顶继续使用既有区域移动动画，不新增成功文字提示；减少动态效果偏好下沿用既有降级策略。`returnHandCardToTop` 只接收当前选中的手牌实例，不再默认处理最右侧手牌。
- 390×844 实测成员行中心误差 0.008px，左右辅助轨均不覆盖成员行；430×932 中心误差 0px，对手镜像同样居中；1600×900 桌面无横向溢出。真实玩家视角确认批量按钮各 27×32px、能量卡 16×24px且控件无溢出。1280×720 实测资源标题均为 `y=385 / h=20`，桌面抽牌与放回按钮同为 64×28px、横坐标完全一致，右下角阶段工具条与两按钮无重叠；390×844 下抽牌按钮位于放回按钮左侧且两者同高。

## 2026-07-23：休息室分类统计与移动端观战弹层层级修复（未提交）

- 休息室统计新增成员卡与 LIVE 卡数量，并与既有 Heart、加分判、抽卡标和无判卡统计共同展示；focused 单测覆盖分类数量与原判心语义。
- 修复移动端观战工具条覆盖休息室关闭按钮和判定区收起按钮：新增 battle chrome / backdrop / modal 层级 token，休息室模态框与全屏判定抽屉明确高于页面 chrome；只读模式仍只禁用规则命令，本地查看与关闭控制保持可用。
- 验证：`tests/unit/waiting-room-judgment-stats.test.ts` 2 tests、客户端 TypeScript、客户端生产构建与 `git diff --check` 通过；390×844 浏览器命中检查确认 modal 160 高于 backdrop 150 与 spectator chrome 120，用户已在真实移动端观战页面确认关闭/收起交互正常；构建仅保留既有大 chunk 警告。

## 2026-07-23：新卡组默认能量、版本复制与操作收纳（未提交）

- 空白新卡组默认填入 12 张 `LL-E-001-SD` 能量卡；共享默认 helper 同时供卡组管理编辑器、deck store 初始化与创建 API 的缺省输入使用，显式传入的能量卡组仍保持用户选择。
- 新增自有云端卡组复制接口与“复制为新版本”入口；副本保留构筑/说明及来源关联、默认关闭分享，名称按同系列现有最高版本自动递增为 `v2 / v3 ...`，创建后直接进入副本编辑器。
- 卡组列表改为只常驻“编辑”和三点菜单，将复制、分享管理与删除收进分层菜单；删除继续二次确认。手机端标题与编辑/更多固定同排，移除“共 N 个卡组”弱信息，并将顶部三个并列入口收束为“创建卡组 + 导入”；DeckLog / YAML 来源改在底部操作单中二次选择，桌面端保留直接入口。
- DeckLog 导入弹层重构为“选择站点 → 输入链接/编号”的明确流程；日本版/国际版展示真实域名，粘贴完整链接时自动切换来源，移动端不再自动唤起键盘。输入区使用卡组编号票据式视觉，补齐可访问弹层标题、关闭按钮、明确错误提示与“读取并导入”主操作，成功后仍直接进入编辑器检查。
- 修复 `pnpm test-env:start` 管理员 seed 仍覆盖为裸 bcrypt 摘要的问题：测试管理员现在写入当前 SHA-256 预处理 bcrypt 格式，并以真实认证 verifier 互验，避免注册/提权后登录持续 401。
- 验证：focused Vitest 3 files / 6 tests、shared/server/client TypeScript、客户端生产构建与 `git diff --check` 通过；卡组管理 focused Playwright 覆盖 390×844、430×932、768×1024、1024×768 四视口，原列表/操作单回归共 18 tests 通过、2 个非手机视口专用用例跳过；DeckLog 弹层本轮 focused 回归另有 10 tests 通过、2 个非手机视口专用用例跳过，截图确认自动来源切换、空输入禁用、弹层无溢出与成功进入编辑器；测试管理员 seed 已在真实本地测试环境完成登录与推荐卡组写入。
