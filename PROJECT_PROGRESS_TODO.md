...
- 轮换主题牌桌第一版开发闭环已落地：独立 `THEME` 队列、活动/预组/组合/分配版本、确认后事务内组合与座位分配、服务端预组锁定、玩家节目单与跨页面候场均已接入现有公共牌桌和正式房间；管理员可通过“活动 → 预组 → 组合 → 发布”工作台冻结版本、发布/暂停/结束活动并即时停用组合，发布后事实不可原地修改；开局前责任方离场或未到场时，无过错方会保留原 FIFO 时间取得可审计的新主题票据。后台已提供基础漏斗计数和目标/实际预组曝光。当前仍是开发/内测基线；真实预组试打矩阵、跨进程房间恢复、完整组合指标/告警和双浏览器验收未完成前不得开放生产活动。
