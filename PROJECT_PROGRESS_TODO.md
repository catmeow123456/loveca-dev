...
- 2026-07-24：补齐对墙打系统席位对旧式隐式单选窗口的兼容。静态盘点 45 个 `AWAITING_PLAYER_ONLY` 私有选择窗口，其中 19 个以未声明 `selectableCardMode` 表达单选；模式自动化按引擎既有语义处理，但只有候选全部仍在系统手牌时才使用位置 token 自动选择，私密检视等非手牌窗口继续安全停止。`PL!-bp5-021` 分数 3「SUNNY DAY SONG」真实流程回归覆盖双方抽 1 弃 1、系统弃牌后回到玩家成员选择，以及隐藏手牌 ID 不进入命令日志。
- 2026-07-24：补齐对墙打系统席位的私有手牌有序多选窄路径。`AWAITING_PLAYER_ONLY + ORDERED_MULTI` 的不可跳过窗口按当前候选顺序选择前 N 张，固定数量取要求值、范围数量取最大合法值；系统命令日志只记录位置 token，权威执行前才解析真实卡牌 ID。`PL!S-bp7-004` 费用 13「黑泽黛雅」与 `PL!-bp5-007` 费用 13「东条希」真实流程回归均覆盖自动结算和隐藏手牌 ID 不进入命令日志。
- 2026-07-24：清理 BP7 卡效的罕度精确卡号技术债。64 条 definition（39 个基础编号）统一迁为 `baseCardCodes`，单卡/shared workflow、continuous registry、费用查询、声援方向、特殊登场与前端勘误均改为基础编号身份；罕度同步测试不再依赖本地 `cards.json` 是否已收录 API-only 新卡，并新增“同基础编号跨罕度命中、相邻编号不串效果、BP7 禁止 exact 登记”的回归护栏。治理 skill、仓库规则与卡效登记文档同步固化“罕度后缀不是卡效边界”。
- 2026-07-24：补齐对墙打系统席位的第一版简单卡效决定。模式自动化通过标准 `CONFIRM_EFFECT_STEP` 命令处理纯确认、可选跳过、强制单张卡牌/选项/成员区选择，并为盲选提交版本化匿名 token；复杂多选、结构化选择、数字、站位和费用窗口继续安全停住。focused policy、真实对手可选卡效、solitaire flow 与 TypeScript 验证覆盖本边界。
