...
const PL_N_BP4_021_ON_ENTER_EFFECT_TEXT = '【登场】可以将自己休息室的1张卡放置于卡组顶。';
...
const SP_BP4_022_LIVE_START_PAY_UP_TO_TWO_ENERGY_GAIN_BLADE_EFFECT_TEXT =
  '【LIVE开始时】可以支付最多2个[E]：LIVE结束时为止，每支付1个[E]，此成员获得1个[BLADE]。';
...
const S_BP6_009_LIVE_SUCCESS_EFFECT_TEXT =
  '【LIVE成功时】因中央声援公开的自己的卡中存在持有 SCORE 图标的『Aqours』LIVE卡的场合，LIVE总分+1。';
...
export const CARD_ABILITY_DEFINITIONS: readonly CardAbilityDefinition[] = [
  ...
  },
  {
    ...
    notes: '复用 discard-then-draw shared workflow；可选弃置1至3张当前手牌，按实际成功弃置数等量抽牌。P/R/PR 同文按基础编号同步。',
  },
  {
    ...
    notes: '复用 discard-then-draw shared workflow；可选弃置恰好2张当前手牌，随后按弃置后的手牌数抽至5张。仅覆盖 PL!HS-PR-031，不覆盖同文 PL!N-PR-028。',
  },
  {
  ...
  },
  {
    ...
    notes:
      '单卡 LIVE_START workflow `pl-bp5-010-honoka.ts`；来源仍在己方舞台且有手牌时打开真实交互弃1手窗口，不套 confirm-only。弃置走 discardOneHandCardToWaitingRoomAndEnqueueTriggers；之后用 moveTopDeckCardsToWaitingRoomWithRefreshAndEnqueueTriggers mill 3 并只为真实进入休息室的牌入队。支付后重扫休息室回收『A-RISE』成员，允许目标来自刚弃手或刚 mill；无目标时费用与 mill 保留并 no-op。',
  },
  {
  ...
  },
  {
    ...
    activatedUi: {
      ...
      title: '支付2能量，从休息室回收1张低分莲之空LIVE卡',
    },
    notes:
  ...
  },
  {
    ...
    notes:
      '单卡 LIVE_START 薄 wrapper；先用可选弃1手 activeEffect shell，弃手通过 enter-waiting-room trigger wrapper 支付并记录，再委托 arrange-inspected-deck-top 检视顶3、任意张有序回顶、余下入休息室。真实交互不套 confirm-only。',
  },
  {
  ...
  },
  {
    ...
    notes:
      '扩展 arrange-inspected-deck-top shared workflow；topCount=2，可选0-2张按顺序回到卡组顶，其余检视卡经 inspection-to-waiting trigger wrapper 放入休息室。PL!HS-bp2-016 与 PL!HS-pb1-024 日文卡文同型，复用同一 ability/workflow。',
  },
  {
  ...
  },
  {
    ...
    notes:
      'shared activated-pay-energy-draw workflow；PL!SP-bp5-020 与 PL!HS-bp1-007 共用支付2张活跃能量抽1，来源仍在自己舞台且支付成功后才记录1回合1次使用。',
  },
  {
  ...
  },
  {
    ...
    activatedUi: {
      ...
      title: '支付2能量并弃1张手牌，回收1张 Aqours LIVE',
    },
  },
  {
  ...
  },
  {
    ...
    notes:
      '单卡 LIVE_SUCCESS workflow；从 eventLog 读取本次自己非 additional 的中央声援公开卡，存在 SCORE Aqours LIVE 时写玩家 LIVE 合计 SCORE +1 并刷新 playerScores。',
  },
  {
  ...
  },
  {
    ...
    notes: '单卡 LIVE_START workflow `n-bp3-009-rina.ts`；恰好2张己方休息室成员按选择顺序置底，按印刷费用 exact 6/8/25 互斥结算抽牌、SOURCE_MEMBER ALL Heart 或不带 liveCardId 的玩家 LIVE 总分 SCORE。',
  },
  {
  ...
  },
  {
    ...
    notes: '单卡 workflow 建立本回合双方 CARD_EFFECT 成员 WAITING -> ACTIVE 限制；RULE_ACTION 不受影响。',
  },
  {
  ...
  },
  {
    ...
    notes: '单卡 workflow `pl-pb1-006-maki.ts`；可选休息室 μ\'s LIVE 置顶后实时重查对方 WAITING 成员并抽1。',
  },
  {
    ...
    perTurnLimit: 1,
    notes: '单卡 workflow `pl-pb1-007-nozomi.ts`；按成功 LIVE 卡数动态降低弃手费用，支付后重查其他 lily white 成员与休息室 μ\'s LIVE。',
    activatedUi: {
      abilityId: PL_PB1_007_ACTIVATED_SUCCESS_COUNT_DISCARD_RECOVER_MUSE_LIVE_ABILITY_ID,
  ...
  },
  {
    ...
    notes:
      '复用 arrange-inspected-deck-top shared workflow 的 target-player 轴；控制者先选择自己或对方，随后控制者查看所选玩家主卡组顶至多2张，任意张按选择顺序回该玩家卡组顶，其余从 MAIN_DECK 进入该玩家休息室并入队等待室触发。来源启动时必须仍在自己的主舞台。',
  },
  {
  ...
  },
  {
    ...
    notes:
      '窄单卡 ON_ENTER workflow `pl-bp3-009-nico.ts`；已入队后不要求来源仍在舞台，结算时重扫自己当前主舞台成员并通过 getMemberEffectiveCost 判定有效费用>=13；无交互 pending 走实时条件 confirm-only，满足时复用 drawCardsForPlayer 抽1。',
  },
  {
  ...
  },
  {
    ...
    notes: '单卡 inspection/reveal workflow `n-bp3-028-tsunagaru-connect.ts`；动态统计虹咲主舞台成员，检视后至多1张回顶、余牌经 inspection-to-waiting wrapper 入休息室，再公开当前顶卡并在确认后回顶；公开 LIVE 时给来源 LIVE 写 SCORE +1。',
  },
  {
  ...
  },
  {
    ...
    notes: '扩展 shared live-start-score-bonuses；读取成功LIVE卡自身印刷分数，按分数1/5是否分别存在写此LIVE SCORE +0/+1/+2。',
  },
  {
  ...
  },
  {
    ...
    notes: '窄单卡 workflow；实时统计自己主舞台三槽 WAITING 成员，写此LIVE SCORE并刷新playerScores。',
  },
  {
  ...
  },
  {
    ...
    notes:
      '单卡 LIVE_SUCCESS workflow `n-bp4-003-shizuku.ts`；无交互 queued pending 走 confirm-only/manual confirmation 语义。确认后实时比较 liveResolution.playerScores，自己 LIVE 合计分数严格高于对方时复用 drawCardsForPlayer 抽1张；未领先或来源离场时消费 pending no-op。',
  },
  {
  ...
  },
  {
    ...
    notes:
      '单卡 ON_ENTER workflow `n-bp4-006-kanata.ts`；真实交互效果不套额外 confirm-only。启动时要求可支付2张活跃能量、有费用<=4的虹ヶ咲手牌成员且有空成员区；选择手牌成员后用 TAP_ACTIVE_ENERGY 支付[E][E]，再选择空成员区登场。登场移动复用 playMemberFromZoneToEmptySlot 并显式入队该成员 ON_ENTER；若因此登场成员持有 BLADE HEART，来源近江彼方通过 member-state changed wrapper 变 WAITING。无能量、无目标、无空位或选择不发动均消费 pending/no-op。',
  },
  {
  ...
  },
  {
    ...
    notes:
      '单卡 ON_ENTER workflow `n-bp4-021-rina.ts`；有休息室目标时打开可选选择窗口，选择1张后复用 moveWaitingRoomCardsToDeckTopForPlayer 放到卡组顶。选择不发动不移动但消费 pending；休息室为空时直接消费 pending no-op。',
  },
  {
  ...
  },
  {
    ...
    activatedUi: {
      ...
      title: '支付2能量并弃1手牌，回收虹咲LIVE',
    },
    notes:
      '单卡 activated workflow `n-bp5-014-kasumi.ts`；主阶段当前玩家、来源仍在己方舞台、2 active energy 和至少1手牌时启动。费用支付2 active energy 并用 discardOneHandCardToWaitingRoomAndEnqueueTriggers 弃1手；弃牌后重扫休息室『虹ヶ咲』LIVE，允许回收刚弃置的 LIVE。无目标时费用保留并 no-op。',
  },
  {
  ...
  },
  {
    ...
    notes: '单卡 wrapper + waiting-room ON_ENTER delegation shared family；仅展示显式 opt-in 的已实现能力。',
  },
  {
