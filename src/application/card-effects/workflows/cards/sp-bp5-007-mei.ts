...
import { SP_BP5_007_ON_ENTER_DISCARD_LOOK_TOP_DISTINCT_GROUPS_ABILITY_ID } from '../../ability-ids.js';
import {
...
function startSpBp5007MeiOnEnter(
  game: GameState,
...
function finishDiscardCostAndStartInspection(
  ...
  if (
    !effect ||
    effect.abilityId !== SP_BP5_007_ON_ENTER_DISCARD_LOOK_TOP_DISTINCT_GROUPS_ABILITY_ID ||
  ...
  return startLookTopSelectToHandWorkflow(
    ...
    },
    {
      ...
      topCount: 5,
      selector: hasKnownGroup,
      countRule: { minCount: 0, maxCount: 3 },
      revealSelectedBeforeHand: true,
      selectStepId: SELECT_TAKE_STEP_ID,
      revealStepId: REVEAL_SELECTED_STEP_ID,
      selectStepText: '请选择至多3张团体名互不重复的卡公开并加入手牌。也可以不加入。',
      noTargetStepText: '没有可加入手牌的团体名卡。确认后其余卡片放置入休息室。',
...
function hasKnownGroup(card: Parameters<typeof getCardGroups>[0]): boolean {
  return getCardGroups(card).length > 0;
}

function hasDistinctGroupAssignment(game: GameState, selectedCardIds: readonly string[]): boolean {
  const groupOptions = selectedCardIds.map((cardId) => {
    const card = getCardById(game, cardId);
    return card ? getCardGroups(card) : [];
  });
  if (groupOptions.some((groups) => groups.length === 0)) {
    return false;
  }

  const assignedGroups = new Set<GroupIdentityName>();
  const assign = (index: number): boolean => {
    if (index >= groupOptions.length) {
      return true;
    }
    for (const groupName of groupOptions[index] ?? []) {
      if (assignedGroups.has(groupName)) {
        continue;
      }
      ...
      assignedGroups.delete(groupName);
    }
    return false;
  };
