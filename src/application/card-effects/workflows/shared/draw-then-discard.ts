...
const DRAW_THEN_DISCARD_WORKFLOWS: readonly {
  ...
  },
  {
    ...
    drawCount: 1,
    discardCount: 1,
    stepId: 'HS_BP6_030_LIVE_START_SELECT_DISCARD',
  },
  {
...
export function startDrawThenDiscardCardsWorkflow(
  ...
  const allowedSourceSlots =
    config.requiredSourceSlots ?? (config.requiredSourceSlot ? [config.requiredSourceSlot] : []);
  if (allowedSourceSlots.length > 0 && !allowedSourceSlots.includes(sourceSlot as SlotPosition)) {
    const state = {
      ...game,
      pendingAbilities: game.pendingAbilities.filter(
        (candidate) => candidate.id !== config.ability.id
