...
export class AiDecisionContext {
  ...
  observe(view: PlayerViewState, observation?: AiPublicObservation): void {
    ...
    const mainDeckCount = view.table.zones[`${this.seat}_MAIN_DECK`].count;
    if (observation) {
      const gap =
      ...
      for (const event of events) {
        ...
        const count = event.type === 'CardMovedPublic' ? (event.count ?? 1) : 1;
        observedDeckDelta += (toDeck ? count : 0) - (fromDeck ? count : 0);
      ...
      // Hidden-to-hidden draws need not emit a public movement. A count mismatch proves
      // unaccounted deck movement, not the identity of the new top. Clear even a top learned
      // in this batch: the unreported movement may have happened after that return.
      if (this.previous && mainDeckCount !== this.previous.mainDeckCount + observedDeckDelta)
        this.knownDeckTop = undefined;
      this.publicSeq = observation.throughPublicSeq;
    } else if (this.previous && mainDeckCount !== this.previous.mainDeckCount) {
      this.knownDeckTop = undefined;
    }
    this.previous = {
      mainDeckCount,
