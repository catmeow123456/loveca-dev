...
const CONTROL = [
  ...
  'LIVE_SET 先读 decisionBrief.liveSet：先核对可唱目标，再确定近期必留牌，最后利用剩余额度周转冗余并确认。printedCheerBaseline 是当前舞台与本构筑印刷声援的乐观基线，不是最终卡效结算；正缺口须指出可执行的补心、增声援或减需求来源，成功后的奖励不能提前补足成功条件。handMembers 按实例列出重复数量，保留多张须分别有近期位置、支付预算或能力用途；高费不自动等于衔接牌。已盖牌数决定本次补抽，额外盖成员不耗能量，不以无LIVE或零能量停止周转。',
  '只输出符合当前 responseSchema 的 JSON：先写最多300字的 tradeoff 预算结论，再写与之对应的 selection（必填）。LIVE_SET 的预算结论包含：表演需求与可补心数、保留牌的使用回合/位置/支付、本次最终盖牌数与补抽数；使用 nextOwnMainBaseline 核对接下来两个自己回合的常规预算，不以当前剩余能量代替；printedPayments 是未计费用修正的差值，必需衔接可保留到第二轮，重复副本须各有用途。确认即结束本次设置，剩余额度作废，不能留到抽牌后使用。一次 selection 只提交一个当前动作，不能声称本次同时盖了别的牌。其他选择简述净资源与下一步用途，结束主要阶段说明放弃的最佳可见路线。不输出逐步推理，不发明卡牌、引用、命令或隐藏信息。',
].join('\n\n');

/** Shared production/QA assembly: variants change materials, never hand-written wire prompts. */
export function buildAiBattleMessages(input: AiDecisionInput, knowledge: AiFrozenKnowledge) {
  return [
    { role: 'system' as const, content: CONTROL },
    ...[knowledge.rules, knowledge.tutorial, knowledge.handbook, knowledge.ownDeck].map(
      (source) => ({
        role: 'user' as const,
