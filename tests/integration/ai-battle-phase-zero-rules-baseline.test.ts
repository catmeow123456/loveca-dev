...
import { BladeHeartEffect, CardType, GamePhase, HeartColor } from '../../src/shared/types/enums';
...
const SYSTEM_PLAYER_ID = 'loveca-ai-standard-v1';
const HUMAN_PLAYER_ID = 'phase-zero-human';
...
const authoritativeCardRegistry = loadAuthoritativeCardRegistry();

describe('AI battle Phase 0 RULES matchup matrix', () => {
  it('loads representative printed stats instead of synthetic smoke values', () => {
    const honoka = authoritativeCardRegistry.getByCode('PL!-sd1-001-SD');
    const flowerKnot = authoritativeCardRegistry.getByCode('PL!HS-bp5-019-L');

    expect(honoka?.cardType).toBe(CardType.MEMBER);
    if (honoka?.cardType === CardType.MEMBER) {
      expect(honoka.cost).toBe(11);
      expect(honoka.blade).toBe(3);
      expect(honoka.hearts).toEqual([
    ...
    expect(flowerKnot?.cardType).toBe(CardType.LIVE);
    if (flowerKnot?.cardType === CardType.LIVE) {
      expect(flowerKnot.score).toBe(6);
      expect(flowerKnot.requirements.totalRequired).toBeGreaterThan(0);
    }
  });

  it.each(AI_BATTLE_PHASE_ZERO_MATCHUP_MATRIX)(
    'boots real deck data and advances the first dual-seat RULES cycle for $scenarioId',
    (scenario) => {
      const aiIsFirst = scenario.aiTurnOrder === 'FIRST';
      const firstPlayerId = aiIsFirst ? SYSTEM_PLAYER_ID : HUMAN_PLAYER_ID;
      const secondPlayerId = aiIsFirst ? HUMAN_PLAYER_ID : SYSTEM_PLAYER_ID;
      ...
      session.createGame(
        `phase-zero-${scenario.scenarioId}`,
        firstPlayerId,
        aiIsFirst ? 'Phase Zero AI' : 'Phase Zero Human',
        secondPlayerId,
        aiIsFirst ? 'Phase Zero Human' : 'Phase Zero AI'
      );
      expect(
        session.initializeGame(loadRuntimeDeck(firstDeckKey), loadRuntimeDeck(secondDeckKey))
          .success
      ).toBe(true);
      ...
      expect(session.executeCommand(createMulliganCommand(firstPlayerId, [])).success).toBe(true);
      if (!aiIsFirst) {
        assertCertifiedAiDecisionSurface(session.state!, SYSTEM_PLAYER_ID, 'MULLIGAN');
      }
      expect(session.executeCommand(createMulliganCommand(secondPlayerId, [])).success).toBe(true);
      ...
      expect(session.executeCommand(createEndPhaseCommand(firstPlayerId)).success).toBe(true);
      expect(session.state?.currentPhase).toBe(GamePhase.MAIN_PHASE);
      if (!aiIsFirst) {
        assertCertifiedAiDecisionSurface(session.state!, SYSTEM_PLAYER_ID, 'MAIN_PHASE');
      }
      expect(session.executeCommand(createEndPhaseCommand(secondPlayerId)).success).toBe(true);
      expect(session.state?.currentPhase).toBe(GamePhase.LIVE_SET_PHASE);
      if (!aiIsFirst) {
        expect(
          session.executeCommand(
            createConfirmStepCommand(firstPlayerId, session.state!.currentSubPhase)
          ).success
        ).toBe(true);
      }
...
function loadRuntimeDeck(deckKey: AiBattlePhaseZeroDeckKey): RuntimeDeckConfig {
  ...
  const result = new DeckLoader(authoritativeCardRegistry).loadFromConfig(deck);
