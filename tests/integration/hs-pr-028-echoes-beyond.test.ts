  ...
  return createCardInstance(
    {
      ...
      cost: 1,
      blade: 1,
      hearts: printedHeartCount > 0 ? [createHeartIcon(HeartColor.PINK, printedHeartCount)] : [],
    },
    ownerId,
    cardCode
  );
...
function live(): ReturnType<typeof createCardInstance> {
  return createCardInstance(
    {
...
function setup(
  extraHeart = true,
  options: {
    readonly printedHeartCount?: number;
    readonly originalHeartReplacementCount?: number;
  } = {}
): { game: GameState; sourceId: string; memberId: string; drawId: string } {
  const source = live();
  const stageMember = member(
    options.printedHeartCount === 0 ? 'PL!N-bp7-003-N' : 'arbitrary-member',
    PLAYER1,
    options.printedHeartCount
  );
  ...
  if (options.originalHeartReplacementCount !== undefined) {
    game = addLiveModifier(game, {
      kind: 'MEMBER_ORIGINAL_HEART_REPLACEMENT',
      playerId: PLAYER1,
      memberCardId: stageMember.instanceId,
      hearts: [createHeartIcon(HeartColor.PINK, options.originalHeartReplacementCount)],
  ...
  return {
    ...
    memberId: stageMember.instanceId,
...
describe('PL!HS-PR-028 Echoes Beyond', () => {
  it('single pending confirms before drawing and shows real-time matching result', () => {
    ...
    const resolveAction = resolved.actionHistory.find(
      (action) => action.type === 'RESOLVE_ABILITY'
    );
    expect(resolveAction?.payload).toMatchObject({
      matchingMemberIds: [memberId],
  ...
  it('does not draw for equal Heart count or a stale LIVE source, without exposing internal source text', () => {
    ...
    const started = resolvePendingCardEffects({
      ...noExtra.game,
      pendingAbilities: [pending(noExtra.sourceId)],
    }).gameState;
    ...
    const stale = updatePlayer(extra.game, PLAYER1, (player) => ({
      ...player,
      liveZone: { ...player.liveZone, cardIds: [] },
    }));
    const staleStarted = resolvePendingCardEffects({
      ...stale,
      pendingAbilities: [pending(extra.sourceId)],
    }).gameState;
    ...
    expect(staleStarted.activeEffect?.effectText).not.toMatch(
      /source|pending|payload|stale|eventId|trigger|LIVE区/
    );
  ...
  it('counts SOURCE_MEMBER bonuses but not opponent members', () => {
    ...
    expect(
      confirm(
        resolvePendingCardEffects({ ...game, pendingAbilities: [pending(base.sourceId)] }).gameState
      ).players[0].hand.cardIds
    ).toContain(base.drawId);
    ...
    game = addLiveModifier(game, {
      ...
      target: 'TARGET_MEMBER',
      playerId: PLAYER2,
      targetMemberCardId: opponent.instanceId,
      hearts: [createHeartIcon(HeartColor.GREEN, 1)],
  ...
  it('treats a count-changing original Heart replacement as the baseline', () => {
    const replacementOnly = setup(false, {
      printedHeartCount: 0,
      originalHeartReplacementCount: 4,
    });
    const replacementOnlyStarted = resolvePendingCardEffects({
      ...replacementOnly.game,
      pendingAbilities: [pending(replacementOnly.sourceId)],
    }).gameState;
    expect(replacementOnlyStarted.activeEffect?.effectText).toContain('未满足条件');
    expect(confirm(replacementOnlyStarted).players[0].hand.cardIds).toEqual([]);

    const withOrdinaryHeart = setup(true, {
      printedHeartCount: 0,
      originalHeartReplacementCount: 4,
    });
    const withOrdinaryHeartStarted = resolvePendingCardEffects({
      ...withOrdinaryHeart.game,
      pendingAbilities: [pending(withOrdinaryHeart.sourceId)],
    }).gameState;
    expect(withOrdinaryHeartStarted.activeEffect?.effectText).toContain('满足条件');
    expect(confirm(withOrdinaryHeartStarted).players[0].hand.cardIds).toContain(
      withOrdinaryHeart.drawId
    );
  });
  ...
  it('ordered resolution consumes two pending effects without confirm-only windows', () => {
    ...
    const selection = resolvePendingCardEffects({
      ...game,
      pendingAbilities: [pending(sourceId, 'first'), pending(sourceId, 'second')],
    }).gameState;
  ...
  it('manual selection opens one confirmation bridge and returns to the real resolver', () => {
    ...
    expect(
      afterConfirm.actionHistory.filter(
        (action) => action.type === 'RESOLVE_ABILITY' && action.payload.pendingAbilityId === 'first'
      )
    ).toHaveLength(1);
  });
});
