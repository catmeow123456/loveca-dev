...
describe('known localized card-text source corrections', () => {
  it('corrects all six upstream color tokens in both CN and JP display text for exact N-bp7-025-SECL', () => {
  ...
  it('is idempotent for already corrected exact text and leaves adjacent card codes untouched', () => {
    expect(
      correctKnownLocalizedCardText('PL!N-bp7-025-SECL', `存在${CORRECT_COLOR_TOKENS}。`)
    ).toBe(`存在${CORRECT_COLOR_TOKENS}。`);
    expect(correctKnownLocalizedCardText('PL!N-bp7-025-L', WRONG_COLOR_TOKENS)).toBe(
      WRONG_COLOR_TOKENS
    );
