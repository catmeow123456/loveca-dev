...
const N_BP7_007_ABOVE_SIX_HEART_ABILITY_ID =
...
const PL_N_PB1_001_CONTINUOUS_ABILITY_ID = 'PL!N-pb1-001:continuous-two-live-cards-gain-two-blade';
...
describe('PL!N-bp7-007-SEC continuous energyBelow red Heart', () => {
  it.each([
    ...
    [7, 1],
    [8, 2],
  ] as const)('counts exactly own energy zone above six: %s', (energyCount, expectedAboveSix) => {
    const setsuna = createCardInstance(
      {
        cardCode: 'PL!N-bp7-007-SEC',
        name: '优木雪菜',
        groupNames: ['虹ヶ咲'],
        cardType: CardType.MEMBER,
        cost: 1,
    ...
    const energies = Array.from({ length: energyCount + 2 }, (_, index) =>
      createCardInstance(
        { cardCode: `E-${energyCount}-${index}`, name: 'Energy', cardType: CardType.ENERGY },
        'p1',
        `energy-${energyCount}-${index}`
      )
    );
    let game = registerCards(createGameState(`n-bp7-007-${energyCount}`, 'p1', 'P1', 'p2', 'P2'), [
      setsuna,
      ...energies,
    ]);
    game = updatePlayer(game, 'p1', (player) => {
      ...
      memberSlots = addEnergyBelowMember(
        memberSlots,
        SlotPosition.CENTER,
        energies[energyCount]!.instanceId
      );
      memberSlots = addEnergyBelowMember(
        memberSlots,
        SlotPosition.CENTER,
        energies[energyCount + 1]!.instanceId
      );
      return {
        ...player,
        memberSlots,
        energyZone: {
          ...player.energyZone,
          cardIds: energies.slice(0, energyCount).map((energy) => energy.instanceId),
          cardStates: new Map(
            energies.slice(0, energyCount).map((energy, index) => [
              energy.instanceId,
              {
    ...
    const modifiers = collectLiveModifiers(game).filter(
      (modifier) => modifier.sourceCardId === setsuna.instanceId
    );
    expect(modifiers).toContainEqual(
      expect.objectContaining({
        kind: 'HEART',
        target: 'SOURCE_MEMBER',
        abilityId: N_BP7_007_BELOW_HEART_ABILITY_ID,
        hearts: [{ color: HeartColor.RED, count: 2 }],
      })
    );
    expect(
      modifiers.filter((modifier) => modifier.abilityId === N_BP7_007_ABOVE_SIX_HEART_ABILITY_ID)
    ).toEqual(
      expectedAboveSix
        ? [
            expect.objectContaining({
              hearts: [{ color: HeartColor.RED, count: expectedAboveSix }],
            }),
          ]
        : []
    );
...
describe('live modifier helpers', () => {
  ...
  it('adds member Heart modifiers without projecting them to player Heart bonuses', () => {
    ...
    const result = addHeartLiveModifierForSourceMember(game, {
      playerId: 'p1',
      sourceCardId: source.instanceId,
    ...
    expect(result?.modifier.target).toBe('SOURCE_MEMBER');
  ...
  it('rejects invalid member Heart helper inputs without modifying state', () => {
    ...
    expect(
      createHeartLiveModifierForTargetMember(game, {
        ...baseOptions,
        targetMemberCardId: 'missing-member',
      })
    ).toBeNull();
    expect(
      createHeartLiveModifierForTargetMember(game, {
        ...baseOptions,
        targetMemberCardId: liveCard.instanceId,
      })
    ).toBeNull();
    expect(
      createHeartLiveModifierForTargetMember(game, {
    ...
    expect(
      addHeartLiveModifierForSourceMember(game, {
        ...baseOptions,
  ...
  it('replaces a member printed original BLADE count before appending BLADE modifiers', () => {
    ...
    expect(getPlayerLiveBladeModifier(game.liveResolution, 'p1')).toBe(4);
  });
  ...
  it('collects exact-seven-energy BLADE plus two across both PR base-code families', () => {
    for (const cardCode of [
      ...
      expect(getPlayerLiveBladeModifier(atSeven.game.liveResolution, 'p1', modifiers)).toBe(0);
  ...
  it('applies the PL!SP-bp7-002 base-family energy-lead stage cost without changing printed selectors', () => {
    for (const [ownEnergyOrientations, opponentEnergyCount, expectedCost] of [
      ...
      [Array(7).fill(OrientationState.ACTIVE), 6, 4],
      [Array(8).fill(OrientationState.ACTIVE), 7, 4],
      [Array(7).fill(OrientationState.ACTIVE), 8, 2],
      [
        [
      ...
      expect(getMemberEffectiveCost(state.game, 'p1', state.sourceId)).toBe(expectedCost);
      expect(costLte(2)(state.game.cardRegistry.get(state.sourceId)!)).toBe(true);
    }
  });
  ...
  it('requires PL!SP-bp7-002 to remain a top-level member on its controller stage', () => {
    for (const sourcePlacement of ['OFF_STAGE', 'MEMBER_BELOW', 'HAND', 'WAITING_ROOM'] as const) {
      ...
      expect(getMemberEffectiveCost(state.game, 'p1', state.sourceId)).toBe(2);
    }

    const sibling = createSpBp7002EffectiveCostState({
      cardCode: 'PL!SP-bp7-002-N',
      ownEnergyOrientations: Array(7).fill(OrientationState.ACTIVE),
      opponentEnergyCount: 6,
    });
  ...
  it('evaluates each PL!SP-bp7-002-P instance from its own controller energy state', () => {
    ...
    const opponentSource = createCardInstance(
      ...
      'p2',
      'sp-bp7-002-opponent-source'
    );
    let game = registerCards(state.game, [opponentSource]);
    game = updatePlayer(game, 'p2', (player) => ({
      ...player,
      memberSlots: placeCardInSlot(
        player.memberSlots,
    ...
    expect(getMemberEffectiveCost(game, 'p1', state.sourceId)).toBe(4);
    expect(getMemberEffectiveCost(game, 'p2', opponentSource.instanceId)).toBe(2);
  });

  it('stacks PL!SP-bp7-002-P before MEMBER_COST and lets MEMBER_COST_SET override without mutation', () => {
    const state = createSpBp7002EffectiveCostState({
      ownEnergyOrientations: Array(7).fill(OrientationState.ACTIVE),
      opponentEnergyCount: 6,
    });
    const withDelta = addMemberCostLiveModifierForMember(state.game, {
      playerId: 'p1',
      memberCardId: state.sourceId,
      sourceCardId: 'test-cost-delta-source',
      abilityId: 'test:sp-bp7-002-cost-plus-three',
      countDelta: 3,
    });
    expect(withDelta).not.toBeNull();
    expect(getMemberEffectiveCost(withDelta!.gameState, 'p1', state.sourceId)).toBe(7);

    const withSet = addMemberCostSetLiveModifierForMember(withDelta!.gameState, {
      playerId: 'p1',
      memberCardId: state.sourceId,
      sourceCardId: 'test-cost-set-source',
      abilityId: 'test:sp-bp7-002-cost-set-one',
      setTo: 1,
    });
    expect(withSet).not.toBeNull();
    const modifierCount = withSet!.gameState.liveResolution.liveModifiers.length;
    expect(getMemberEffectiveCost(withSet!.gameState, 'p1', state.sourceId)).toBe(1);
    expect(getMemberEffectiveCost(withSet!.gameState, 'p1', state.sourceId)).toBe(1);
    expect(withSet!.gameState.liveResolution.liveModifiers).toHaveLength(modifierCount);
  });
...
describe('PL!S-bp7-016-N 费用15「国木田花丸」 continuous Heart', () => {
  it('只计己方三个主舞台顶层成员，并为来源动态提供红绿蓝 Heart 各1', () => {
    ...
    let game = registerCards(createGameState('s-bp7-016-continuous', 'p1', 'P1', 'p2', 'P2'), [
      source,
      allyLeft,
      allyRight,
      below,
    ...
    const abilityId = 'PL!S-bp7-016-N:continuous-stage-three-gain-red-green-blue-heart';
...
describe('PL!SP-bp7-013-N 费用15「唐可可」 continuous KALEIDOSCORE Heart/BLADE', () => {
  it('只统计己方主舞台顶层 KALEIDOSCORE，并只为来源动态提供紫 Heart 与 BLADE', () => {
    const createMember = (
      cardCode: string,
      ownerId: string,
      instanceId: string,
      ...
      createCardInstance(
        {
          ...
          unitName,
          cardType: CardType.MEMBER,
          cost: cardCode === 'PL!SP-bp7-013-N' ? 15 : 4,
          blade: 1,
          hearts: [createHeartIcon(HeartColor.PINK, 1)],
        },
        ownerId,
    ...
    const source = createMember('PL!SP-bp7-013-N', 'p1', 'sp-bp7-013-source', 'KALEIDOSCORE');
    const allyOne = createMember('ALLY-ONE', 'p1', 'sp-bp7-013-ally-one', 'KALEIDOSCORE');
    const allyTwo = createMember('ALLY-TWO', 'p1', 'sp-bp7-013-ally-two', 'KALEIDOSCORE');
    const outsider = createMember('OUTSIDER', 'p1', 'sp-bp7-013-outsider', 'CatChu!');
    const below = createMember('BELOW', 'p1', 'sp-bp7-013-below', 'KALEIDOSCORE');
    const opponent = createMember('OPPONENT', 'p2', 'sp-bp7-013-opponent', 'KALEIDOSCORE');
    let game = registerCards(createGameState('sp-bp7-013-continuous', 'p1', 'P1', 'p2', 'P2'), [
      source,
      allyOne,
      allyTwo,
      outsider,
    ...
    game = placeMemberOnStage(game, 'p1', SlotPosition.CENTER, source.instanceId);
    game = placeMemberOnStage(game, 'p1', SlotPosition.LEFT, allyOne.instanceId);
    game = placeMemberOnStage(game, 'p1', SlotPosition.RIGHT, outsider.instanceId);
    game = placeMemberOnStage(game, 'p2', SlotPosition.CENTER, opponent.instanceId);
    game = updatePlayer(game, 'p1', (player) => ({
      ...player,
      memberSlots: addMemberBelowMember(player.memberSlots, SlotPosition.CENTER, below.instanceId),
    ...
    const abilityId = 'PL!SP-bp7-013-N:continuous-three-kaleidoscore-gain-purple-heart-blade';
    const abilityModifiers = (state: GameState) =>
      collectLiveModifiers(state).filter((modifier) => modifier.abilityId === abilityId);

    expect(abilityModifiers(game)).toEqual([]);

    game = placeMemberOnStage(game, 'p1', SlotPosition.RIGHT, allyTwo.instanceId);
    const modifiers = abilityModifiers(game);
    ...
    expect(modifiers.find((modifier) => modifier.kind === 'BLADE')).not.toHaveProperty(
      'targetMemberCardId'
    );
    expect(
      getPlayerLiveHeartModifiers(game.liveResolution, 'p1', collectLiveModifiers(game))
    ...
    expect(getMemberEffectiveBladeCount(game, 'p1', allyOne.instanceId)).toBe(1);
    expect(getMemberEffectiveBladeCount(game, 'p1', allyTwo.instanceId)).toBe(1);
    expect(abilityModifiers(game)).toHaveLength(2);

    const necessaryMemberLeft = updatePlayer(game, 'p1', (player) => ({
      ...player,
      memberSlots: removeCardFromSlot(player.memberSlots, SlotPosition.RIGHT),
    }));
    expect(abilityModifiers(necessaryMemberLeft)).toEqual([]);

    const sourceLeft = updatePlayer(game, 'p1', (player) => ({
      ...player,
      memberSlots: removeCardFromSlot(player.memberSlots, SlotPosition.CENTER),
    }));
    expect(abilityModifiers(sourceLeft)).toEqual([]);
  });
});
...
describe('PL!N-pb1-002 continuous energyBelow LIVE total score', () => {
  function setupKasumiScoreScenario(
    options: {
    ...
    let game = registerCards(createGameState('n-pb1-002-continuous', 'p1', 'P1', 'p2', 'P2'), [
      kasumi,
      second,
      host,
      belowMember,
  ...
  it('stacks independently across sources and follows a source with its energyBelow to a new slot', () => {
    ...
    expect(
      scoreModifiers(game)
        ...
        .sort()
    ).toEqual([kasumi.instanceId, second.instanceId].sort());
    expect(getPlayerLiveScoreModifier(game.liveResolution, 'p1', collectLiveModifiers(game))).toBe(
      2
    );
...
describe("PL!-bp4-020-L 成功区常时 CENTER μ's BLADE", () => {
  ...
  it.each([OrientationState.ACTIVE, OrientationState.WAITING])(
    ...
    (orientation) => {
      const { game, sources, center } = setupLoveWingBell({ orientation });
      const modifiers = collectLiveModifiers(game);
      expect(modifiersFor(game)).toEqual([
        expect.objectContaining({
          ...
          countDelta: 1,
          sourceCardId: sources[0]!.instanceId,
          targetMemberCardId: center.instanceId,
          abilityId,
        }),
      ]);
      expect(getMemberEffectiveBladeCount(game, 'p1', center.instanceId, modifiers)).toBe(3);
      expect(getPlayerLiveBladeModifier(game.liveResolution, 'p1', modifiers)).toBe(0);
      expect(game.liveResolution.liveModifiers).toEqual([]);
  ...
  it("中央成员移到侧边时立即消失，新的中央 μ's 成员立即获得", () => {
    const { game, sources, center, side } = setupLoveWingBell();
    ...
    expect(modifiersFor(swapped)).toEqual([
      expect.objectContaining({
        target: 'TARGET_MEMBER',
        sourceCardId: sources[0]!.instanceId,
        targetMemberCardId: side.instanceId,
        countDelta: 1,
      }),
    ]);
...
function createSpBp7002EffectiveCostState(options: {
  readonly cardCode?: string;
  readonly ownEnergyOrientations: readonly OrientationState[];
  readonly opponentEnergyCount: number;
  readonly sourcePlacement?: 'MAIN_STAGE' | 'OFF_STAGE' | 'MEMBER_BELOW' | 'HAND' | 'WAITING_ROOM';
}) {
  const cardCode = options.cardCode ?? 'PL!SP-bp7-002-P';
  const source = createCardInstance(
    {
      cardCode,
      name: '唐可可',
  ...
  const host = createStageMember('PL!SP-bp7-test-host', 'p1', 'sp-bp7-002-host', 2);
  const ownEnergyCards = options.ownEnergyOrientations.map((_, index) =>
    createCardInstance(
      {
        cardCode: `PL!SP-bp7-002-own-energy-${index}`,
        name: `Own Energy ${index}`,
        cardType: CardType.ENERGY,
      },
      'p1',
      `sp-bp7-002-own-energy-${index}`
    )
  );
  const opponentEnergyCards = Array.from({ length: options.opponentEnergyCount }, (_, index) =>
    createCardInstance(
      {
        cardCode: `PL!SP-bp7-002-opponent-energy-${index}`,
        name: `Opponent Energy ${index}`,
        cardType: CardType.ENERGY,
      },
      'p2',
      `sp-bp7-002-opponent-energy-${index}`
    )
  );
  let game = registerCards(createGameState('sp-bp7-002-effective-cost', 'p1', 'P1', 'p2', 'P2'), [
    source,
    host,
    ...ownEnergyCards,
    ...opponentEnergyCards,
  ...
  game = updatePlayer(game, 'p1', (player) => {
    ...
    return {
      ...
      energyZone: ownEnergyCards.reduce(
        (zone, card, index) =>
          addCardToStatefulZone(zone, card.instanceId, {
            orientation: options.ownEnergyOrientations[index],
            face: FaceState.FACE_UP,
          }),
        player.energyZone
      ),
...
describe('BP7 memberBelow target-aware modifiers', () => {
  ...
  it('keeps BLADE ability source separate from its target and cleans up by target', () => {
    ...
    let game = registerCards(createGameState('target-aware-blade', 'p1', 'P1', 'p2', 'P2'), [
      source,
      target,
    ]);
    game = updatePlayer(game, 'p1', (player) => ({
  ...
  it('rejects a target-aware BLADE target that is not the current own top-level stage member', () => {
    ...
    const game = registerCards(createGameState('target-aware-guard', 'p1', 'P1', 'p2', 'P2'), [
      source,
      target,
    ]);
    expect(
  ...
  it('replaces the complete printed Heart vector, appends bonuses, and uses the latest replacement', () => {
    ...
    game = addHeartLiveModifierForSourceMember(game, {
      playerId: 'p1',
      sourceCardId: member.instanceId,
      abilityId: 'bonus',
      hearts: [{ color: HeartColor.YELLOW, count: 1 }],
