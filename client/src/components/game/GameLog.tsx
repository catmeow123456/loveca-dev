...
export const GameLogContent = memo(function GameLogContent({ active = true }: GameLogContentProps) {
  ...
  return (
    <>
      ...
      >
        <AnimatePresence initial={false}>
          {logs.map((log) => (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              className={cn(
                'rounded border border-[var(--border-subtle)] px-2 py-1',
        ...
        {logs.length === 0 && (
          <div className="py-4 text-center text-[var(--text-muted)]">暂无日志</div>
        )}
      </div>
    </>
  );
    ...
    <div className="fixed left-0 top-0 z-[var(--z-game-log)] flex h-full">
      ...
      <AnimatePresence>
        {isExpanded && (
          ...
          >
            <GameLogContent active={isExpanded} />
