...
export function CardEditModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isCreating) {
        if (
          !currentFormData.cardCode ||
          !currentFormData.cardType ||
          !(currentFormData.nameCn?.trim() || currentFormData.nameJp?.trim())
        ) {
          throw new Error('请填写卡牌编号、中文或日文名称和类型');
        }
        await onCreate({
          cardCode: currentFormData.cardCode,
          cardType: currentFormData.cardType,
          nameJp: currentFormData.nameJp,
          nameCn: currentFormData.nameCn,
          workNames: currentFormData.workNames,
          groupNames: currentFormData.groupNames,
          unitName: currentFormData.unitName,
          unitNameRaw: currentFormData.unitNameRaw,
          ...
          requirements:
            currentFormData.cardType === 'LIVE' ? currentFormData.requirements : undefined,
          cardTextJp: currentFormData.cardTextJp,
          cardTextCn: currentFormData.cardTextCn,
          imageFilename,
          imageSourceUri: currentFormData.imageSourceUri,
          rare: currentFormData.rare,
          product: currentFormData.product,
          productCode: currentFormData.productCode,
          sourceExternalId: currentFormData.sourceExternalId,
          sourceFlags: currentFormData.sourceFlags,
        });
      } else if (card) {
  ...
  const selectedGroups = formData.groupNames ?? [];
  ...
  const toggleGroup = (group: string) => {
    ...
    const groupsForUnitLookup = nonEmptyGroups.filter((g) => g.trim() || g === '');
    const newUnitOptions =
      groupsForUnitLookup.length > 0
        ? [...new Set(groupsForUnitLookup.flatMap((g) => GROUP_UNIT_MAP[g] || []))]
        : [];
    const currentUnitValid = newUnitOptions.includes(formData.unitName || '');
    setFormData({
      ...formData,
      groupNames: nonEmptyGroups.length > 0 ? nonEmptyGroups : null,
            ...
            <div className="space-y-4">
              <section className={sectionClass}>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div>
                    <label className={fieldLabelClass}>中文卡名</label>
                    <input
                      type="text"
                      value={formData.nameCn || ''}
                      onChange={(e) => setFormData({ ...formData, nameCn: e.target.value || null })}
                      className={inputClass}
                    />
                  </div>
                  <div>
                    <label className={fieldLabelClass}>日文卡名</label>
                    <input
                      type="text"
                      value={formData.nameJp || ''}
                      onChange={(e) => setFormData({ ...formData, nameJp: e.target.value || null })}
                      className={inputClass}
                    />
                  </div>
                </div>
              ...
              <section className={sectionClass}>
                <div className={sectionTitleClass}>团体与收录</div>
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {selectedGroups.length > 0 ? (
                        ...
                        <div className="py-1 text-sm text-[var(--text-muted)]">
                          点击上方标签添加真实团体
                        </div>
                      )}
                    </div>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, blade: parseInt(e.target.value) || 0 })
                        }
                        className={inputClass}
                        min="0"
                      />
                      ...
                      {(formData.bladeHearts || []).map((item, i) => {
                        ...
                        return (
                          <div key={i} className={selectedChipClass}>
                            {pillColor && (
                              <span
                                className={`inline-block w-2.5 h-2.5 rounded-full ${pillColor}`}
                              />
                            )}
                            <span>{pillLabel}</span>
                            <button
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
                  ...
                  >
                    {imagePreview ? (
                      ...
                      <img
                        src={resolveCardImagePath(card, 'medium')}
                        alt={getCardLocalizedInfo(card).title}
