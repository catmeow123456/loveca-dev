...
SELECT
  ...
  COALESCE((SELECT jsonb_agg(jsonb_build_object(
    ...
    'ratedMatchCount', rating.rated_match_count
...
export function buildRankedVolatilityReport(payload, generatedAt = new Date()) {
  ...
  if (seasons.length > 1) throw new Error('multiple active/finalizing seasons found; pass --season-id');
  const season = seasons[0];
  const config = parseConfig(season.rating_config);
  const names = new Map(array(payload.players).map((player) => [player.userId, safeName(player.name)]));
  ...
  const seeds = new Map(array(payload.seeds).map((seed) => [seed.userId, {
    rating: number(seed.rating), ratingDeviation: number(seed.ratingDeviation), ratedMatchCount: 0, lastRatedAt: null,
  }]));
  ...
  const loserChanges = [];
  const asymmetry = [];
  const changeBands = new Map([['第 1 局', []], ['第 2～3 局', []], ['第 4～9 局', []], ['第 10 局及以后', []]]);
  ...
  for (const event of effectiveEvents) {
    ...
    const rated = rateHeadToHead(firstBefore, secondBefore, event.winnerSeat === 'FIRST' ? 1 : 0, event.ratedAt, config);
    ...
    recordPlayerMatch(playerStats, event.firstUserId, event.secondUserId, event.winnerSeat === 'FIRST');
    recordPlayerMatch(playerStats, event.secondUserId, event.firstUserId, event.winnerSeat === 'SECOND');
  }
  ...
  for (const [userId, state] of players) {
    ...
    if (!stored || Math.abs(number(stored.rating) - state.rating) > 0.001 ||
  ...
  for (const userId of projection.keys()) if (!players.has(userId)) projectionMismatches.push(userId);

  const leaderboard = [...players.entries()].map(([userId, state]) => {
    ...
    return {
      ...
      rating: round(state.rating, 3), displayedRating: Math.round(state.rating),
      ratingDeviation: round(state.ratingDeviation, 3), matches: state.ratedMatchCount,
      wins: stats.wins, losses: stats.losses, winRate: ratio(stats.wins, state.ratedMatchCount),
  ...
  const stabilityBands = [['1～3 场', 1, 3], ['4～9 场', 4, 9], ['10 场以上', 10, Infinity]].map(([label, low, high]) => {
    const entries = leaderboard.filter((player) => player.matches >= low && player.matches <= high);
    return { label, players: entries.length, ratingDeviation: numericSummary(entries.map((p) => p.ratingDeviation)), rating: numericSummary(entries.map((p) => p.rating)) };
  });

  return {
    schemaVersion: REPORT_SCHEMA_VERSION,
    generatedAt: generatedAt.toISOString(),
    privacy: 'Includes display names/usernames by operator approval; excludes emails, user IDs, match IDs, decks, rooms, and replay payloads.',
    season: {
      seasonKey: season.season_key, name: season.name, lifecycle: season.lifecycle,
      queueAdmission: season.queue_admission, startsAt: iso(season.starts_at), scheduledEndsAt: iso(season.scheduled_ends_at),
      algorithmVersion: season.rating_algorithm_version, leaderboardMinimumMatchCount: leaderboardMinimum,
      configuredPlacementMatchCount: config.placementMatchCount, ledgerRevision: Number(season.ledger_revision),
    },
    algorithm: publicConfig(config),
    input: { source: 'ranked tables + public player display names, one repeatable-read read-only SELECT', matchStatusCounts: payload.match_status_counts ?? {} },
    ratingLedger: {
      totalEvents: events.length, effectiveRatedMatches: effectiveEvents.length,
    ...
    perMatchChange: {
      ...
      winnerGain: numericSummary(winnerChanges), loserLoss: numericSummary(loserChanges),
      twoPlayerAbsoluteDeltaGap: numericSummary(asymmetry),
      byPlayerPreMatchCount: Object.fromEntries([...changeBands.entries()].map(([label, values]) => [label, numericSummary(values)])),
    },
    playerStability: { playerCount: leaderboard.length, matchBands: stabilityBands, earlySampleLeaders: earlyLeaders },
...
function resolveEffectiveEvents(events, algorithmVersion) {
  ...
  for (const event of [...events].sort((a, b) => a.eventSequence - b.eventSequence || a.id.localeCompare(b.id))) {
    if (event.algorithmVersion !== algorithmVersion) throw new Error('rating event algorithm version mismatch');
  ...
  return [...latestByMatch.values()]
    ...
    .sort((a, b) => a.ratedAt.getTime() - b.ratedAt.getTime() || a.matchId.localeCompare(b.matchId) || a.eventSequence - b.eventSequence);
}

function parseConfig(raw) {
  const config = typeof raw === 'string' ? JSON.parse(raw) : raw;
  const keys = ['initialRating', 'initialRatingDeviation', 'minimumRatingDeviation', 'maximumRatingDeviation', 'inactivityTimeUnitMs', 'deviationIncreasePerTimeUnit', 'placementMatchCount'];
  for (const key of keys) if (!Number.isFinite(Number(config?.[key]))) throw new Error(`invalid rating_config.${key}`);
  if (config.ratingPeriodMode !== 'PER_MATCH') throw new Error('only PER_MATCH Glicko seasons are supported');
  return config;
}

function publicConfig(config) {
  return {
    initialRating: Number(config.initialRating), initialRatingDeviation: Number(config.initialRatingDeviation),
    minimumRatingDeviation: Number(config.minimumRatingDeviation), maximumRatingDeviation: Number(config.maximumRatingDeviation),
    inactivityTimeUnitMs: Number(config.inactivityTimeUnitMs), deviationIncreasePerTimeUnit: Number(config.deviationIncreasePerTimeUnit),
    placementMatchCount: Number(config.placementMatchCount), displayDecimalPlaces: Number(config.displayDecimalPlaces),
  };
}

function initialState(config) { return { rating: Number(config.initialRating), ratingDeviation: Number(config.initialRatingDeviation), ratedMatchCount: 0, lastRatedAt: null }; }
function cloneState(state) { return { ...state, lastRatedAt: state.lastRatedAt ? new Date(state.lastRatedAt) : null }; }
function rateHeadToHead(first, second, firstScore, ratedAt, config) {
  ...
  const preparedSecond = { ...second, ratingDeviation: increasedDeviation(second, ratedAt, config) };
...
function increasedDeviation(state, ratedAt, config) {
  ...
  return Math.min(Number(config.maximumRatingDeviation), Math.max(Number(config.minimumRatingDeviation), Math.sqrt(state.ratingDeviation ** 2 + Number(config.deviationIncreasePerTimeUnit) ** 2 * elapsed / Number(config.inactivityTimeUnitMs))));
}
function ratePeriod(player, opponent, score, config) {
  const impact = 1 / Math.sqrt(1 + (3 * GLICKO_Q ** 2 * opponent.ratingDeviation ** 2) / Math.PI ** 2);
  const expected = 1 / (1 + 10 ** ((-impact * (player.rating - opponent.rating)) / 400));
  const variance = 1 / (GLICKO_Q ** 2 * impact ** 2 * expected * (1 - expected));
  const precision = 1 / player.ratingDeviation ** 2 + 1 / variance;
  return { rating: player.rating + (GLICKO_Q / precision) * impact * (score - expected), ratingDeviation: Math.min(Number(config.maximumRatingDeviation), Math.max(Number(config.minimumRatingDeviation), Math.sqrt(1 / precision))) };
}
...
function assignRanks(leaderboard, minimum) { let rank = 0; for (const player of leaderboard) player.rank = player.matches >= minimum ? ++rank : null; }
function matchBand(matches) { return matches === 0 ? '第 1 局' : matches <= 2 ? '第 2～3 局' : matches <= 8 ? '第 4～9 局' : '第 10 局及以后'; }
function normalizeEvent(event) { return { id: String(event.id), eventSequence: Number(event.eventSequence), eventType: String(event.eventType), matchId: String(event.matchId), targetEventId: event.targetEventId === null ? null : String(event.targetEventId), firstUserId: String(event.firstUserId), secondUserId: String(event.secondUserId), winnerSeat: event.winnerSeat === null ? null : String(event.winnerSeat), ratedAt: new Date(event.ratedAt), algorithmVersion: String(event.algorithmVersion) }; }
function numericSummary(values) { if (!values.length) return null; const sorted = [...values].sort((a, b) => a - b); const sum = sorted.reduce((total, value) => total + value, 0); return { min: round(sorted[0], 3), p10: round(quantile(sorted, .1), 3), p25: round(quantile(sorted, .25), 3), median: round(quantile(sorted, .5), 3), p75: round(quantile(sorted, .75), 3), p90: round(quantile(sorted, .9), 3), p95: round(quantile(sorted, .95), 3), max: round(sorted.at(-1), 3), mean: round(sum / sorted.length, 3) }; }
function quantile(sorted, p) { if (sorted.length === 1) return sorted[0]; const pos = (sorted.length - 1) * p; const low = Math.floor(pos); const high = Math.ceil(pos); return low === high ? sorted[low] : sorted[low] + (sorted[high] - sorted[low]) * (pos - low); }
function ratio(n, d) { return d === 0 ? null : round(n / d, 4); }
function round(value, places) { const factor = 10 ** places; return Math.round(value * factor) / factor; }
function array(value) { return Array.isArray(value) ? value : typeof value === 'string' ? JSON.parse(value) : []; }
function number(value) { const parsed = Number(value); if (!Number.isFinite(parsed)) throw new Error('expected finite number'); return parsed; }
function iso(value) { return new Date(value).toISOString(); }
function safeName(value) { return typeof value === 'string' && value.trim() ? value.trim() : '（未命名）'; }

export function formatRankedVolatilityReportMarkdown(report) {
  const lines = [
    '# Loveca 排位积分波动报告', '',
    `- 生成时间：${report.generatedAt}`, `- 赛季：${escapeMarkdown(report.season.name)}（${escapeMarkdown(report.season.seasonKey)}）`,
    `- 算法：\`${report.season.algorithmVersion}\``, `- 数据库事务：REPEATABLE READ / READ ONLY`,
    ...
    summaryRow('胜者加分', report.perMatchChange.winnerGain), summaryRow('负者扣分绝对值', report.perMatchChange.loserLoss),
    summaryRow('同局双方绝对变化差', report.perMatchChange.twoPlayerAbsoluteDeltaGap), '',
    '### 按赛前场次', '', '| 场次 | 中位数 | P90 | 最大值 |', '| --- | ---: | ---: | ---: |',
    ...Object.entries(report.perMatchChange.byPlayerPreMatchCount).map(([label, value]) => `| ${label} | ${summaryValue(value, 'median')} | ${summaryValue(value, 'p90')} | ${summaryValue(value, 'max')} |`), '',
    '## 参榜门槛情景', '', '| 门槛 | 可上榜人数 | 第一名 | 积分 | RD | 场次 |', '| ---: | ---: | --- | ---: | ---: | ---: |',
    ...report.leaderboardScenarios.map((scenario) => `| ${scenario.minimumMatches} | ${scenario.eligiblePlayers} | ${escapeMarkdown(scenario.leader?.player ?? '—')} | ${scenario.leader?.displayedRating ?? '—'} | ${scenario.leader ? round(scenario.leader.ratingDeviation, 1) : '—'} | ${scenario.leader?.matches ?? '—'} |`), '',
    '## 当前榜首与短样本', '', '| 排名 | 玩家 | 积分 | RD | 场次 | 胜-负 | 对手数 |', '| ---: | --- | ---: | ---: | ---: | --- |',
    ...report.leaderboard.slice(0, 20).map(playerRow), '',
    '### 仅 1～3 场玩家中的最高积分者', '', '| 玩家 | 积分 | RD | 场次 | 胜-负 |', '| --- | ---: | ---: | ---: | ---: |',
    ...report.playerStability.earlySampleLeaders.map((player) => `| ${escapeMarkdown(player.player)} | ${player.displayedRating} | ${round(player.ratingDeviation, 1)} | ${player.matches} | ${player.wins}-${player.losses} |`), '',
    '## RD 稳定性', '', '| 场次 | 玩家数 | RD 中位数 | RD P90 | RD 最大值 |', '| --- | ---: | ---: | ---: | ---: |',
    ...report.playerStability.matchBands.map((band) => `| ${band.label} | ${band.players} | ${summaryValue(band.ratingDeviation, 'median')} | ${summaryValue(band.ratingDeviation, 'p90')} | ${summaryValue(band.ratingDeviation, 'max')} |`), '',
    '## 说明', '',
    ...report.limitations.map((item) => `- ${item}`), '',
  ];
  return lines.join('\n');
}
function summaryRow(label, value) { return `| ${label} | ${summaryValue(value, 'median')} | ${summaryValue(value, 'p90')} | ${summaryValue(value, 'p95')} | ${summaryValue(value, 'max')} |`; }
function summaryValue(summary, key) { return summary ? summary[key] : '—'; }
function playerRow(player) { return `| ${player.rank ?? '定位中'} | ${escapeMarkdown(player.player)} | ${player.displayedRating} | ${round(player.ratingDeviation, 1)} | ${player.matches} | ${player.wins}-${player.losses} | ${player.uniqueOpponents} |`; }
function escapeMarkdown(value) { return String(value).replaceAll('|', '\\|').replaceAll('\n', ' '); }

function parseArgs(rawArgs) {
  const args = { seasonId: null, outputDir: 'ranked-volatility-output', statementTimeoutMs: 60_000, help: false };
  for (const arg of rawArgs) {
    if (arg === '--help' || arg === '-h') { args.help = true; continue; }
    if (!arg.startsWith('--') || !arg.includes('=')) throw new Error(`options must use --name=value form: ${arg}`);
    const [name, value] = [arg.slice(0, arg.indexOf('=')), arg.slice(arg.indexOf('=') + 1)];
    if (name === '--season-id') { if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value)) throw new Error('--season-id must be a UUID'); args.seasonId = value; }
    else if (name === '--output-dir') { if (!value) throw new Error('--output-dir must not be empty'); args.outputDir = value; }
    else if (name === '--statement-timeout-ms') { const timeout = Number(value); if (!Number.isInteger(timeout) || timeout < 1_000 || timeout > 300_000) throw new Error('--statement-timeout-ms must be an integer from 1000 to 300000'); args.statementTimeoutMs = timeout; }
    else throw new Error(`unknown option: ${name}`);
  }
  return args;
}
function isDirectExecution() { return Boolean(process.argv[1]) && pathToFileURL(path.resolve(process.argv[1])).href === import.meta.url; }
