...
- 当前末端顺序为 `0010_add_ranked_system.sql`、`0011_add_match_replay_retention.sql`、`0012_add_ai_battle_system_fields.sql`；AI 增量以 `0011_snapshot.json` 为前驱，不与排位迁移复用编号。
- `docker/init.sql` 是本地开发和新库初始化的基础启动脚本，并承载 Drizzle schema 不表达的函数与触发器。数据库存在后，后续结构变化应通过 `drizzle/` 迁移进入。
- 已由增量迁移创建的表、索引、约束或函数，不得再单独回写进 `docker/init.sql`；否则全新数据库会先由 init 创建对象，再因空迁移记录重复执行对应迁移。只有在单独重建迁移基线并同步验证新库和现有库路径时，才能把增量结构收回 init。
