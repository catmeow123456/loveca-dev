...
适用迁移：`drizzle/0021_add_ai_battle_system_fields.sql`

执行前提：已经按 journal 顺序完成至 `0020_add_ranked_deck_observations.sql` 的全部迁移。
本迁移的 snapshot 前驱是 `0020_snapshot.json`，不得把旧分支上的 AI `0010` / `0012`
改名后直接执行。
